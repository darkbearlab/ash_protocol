...
let game=loadGame()||new Game(),renderer=new Renderer($('#battle'),game),lockUntil=0,lastStatus='playing',previousFloor=game.floor,noticeTimer;
audio.enabled=read('ash-sound')!=='off';
const notice=document.createElement('div');notice.className='battle-notice';notice.setAttribute('role','status');$('#viewport').append(notice);
const aimBanner=document.createElement('div');aimBanner.className='aim-banner';aimBanner.hidden=true;$('#viewport').append(aimBanner);
const contextBar=document.createElement('div');contextBar.className='context-actions';$('#viewport').append(contextBar);
...
function update() {
  ...
  $('#sector-title').textContent=`${pad(game.floor)} / ${FLOORS[game.floor-1]}`;$('#sector-number').textContent=pad(game.floor);
  $('.sector-sub').textContent=FLOOR_INFO[game.floor-1].subtitle;$('#seed-label').textContent=`${TEST_MODE?'QA':'RUN'} ${String(game.seed).padStart(6,'0')}`;
  $('#turn').textContent=String(game.turn).padStart(3,'0');$('#hp').innerHTML=`${p.hp} <em>/ ${p.maxHp}</em>`;
  $('#hp-bar').style.width=$('#mobile-hp-bar').style.width=`${p.hp/p.maxHp*100}%`;$('#mobile-hp').textContent=`${p.hp} / ${p.maxHp}`;
  $('#level').textContent=`LV. ${pad(p.level)}`;$('#xp').textContent=`${p.xp} / ${p.level+2}`;$('#xp-bar').style.width=`${p.xp/(p.level+2)*100}%`;
  $('#weapon-tag').textContent=`${p.owned.indexOf(p.weapon)+1} / ${p.owned.length}`;
  const src=`./assets/${w.file}.svg`;if($('#weapon-image').getAttribute('src')!==src)$('#weapon-image').src=src;
  $('#weapon-image').alt=w.name;$('#weapon-code').textContent=w.code;$('#weapon-name').textContent=w.name+(p.upgrades[p.weapon]?` +${p.upgrades[p.weapon]}`:'');$('#weapon-type').textContent=w.type;
  $('#damage').textContent=`${damage.min}–${damage.max}${w.burst?' ×2':''}`;$('#range').innerHTML=`${w.range} <em>格</em>`;
  $('#ammo').innerHTML=`${p.ammo[p.weapon]} <em>/ ${reserve}</em>`;
  $('#ammo-pips').innerHTML=Array.from({length:w.mag},(_,i)=>`<i class="${i>=p.ammo[p.weapon]?'empty':''}"></i>`).join('');
  $('#med-count').textContent=p.meds;$('#reload-label').textContent='R';
  ...
  $('#status-effects').textContent=[exposed?`暴露 ${exposed}`:threats.length?'掩護':game.cover.length?'牆 / 箱旁':'',p.moved?'移動':'',p.guard?'防禦':'',p.poison?`中毒 ${p.poison}`:''].filter(Boolean).join(' · ');
  ...
  const aim=target&&def?game.accuracy(p,target):null;
  ...
  $('#interact').hidden=distance(p,game.end)>1;$('#interact').textContent=game.bossAlive?'電梯鎖定 · 清除頭目':game.floor===FLOORS.length?'啟動撤離 ↑':'搭乘電梯 ↓';
  $('#floor-count').textContent=`${pad(game.floor)} / ${pad(FLOORS.length)}`;document.querySelectorAll('.floor-progress i').forEach((el,i)=>el.classList.toggle('current',i<game.floor));
  $('#objective-title').textContent=game.bossAlive?'擊敗本層頭目':game.floor===FLOORS.length?'前往電梯並撤離':'找到下一層入口';$('#objective-text').textContent=FLOOR_INFO[game.floor-1].text;
  $('#log').replaceChildren(...game.logs.slice(0,3).map(l=>{const el=document.createElement('p'),time=document.createElement('time');time.textContent=String(l.turn).padStart(3,'0');el.append(time,document.createTextNode(l.text));if(l.danger)el.className='danger';return el;}));
  contextBar.innerHTML=(game.nearbyTerminal?'<button data-action="terminal">▤ 補給終端</button>':'')+(game.groundWeapon?'<button data-action="bag">▣ 附近有武器</button>':'');
  updateAim();
  ...
  else if(game.pendingPerks&&game.status==='playing')showPerks();
}
function act(type,arg) {
  if($('#modal').open||performance.now()<lockUntil)return false;
...
function floorToast(){const el=$('#floor-toast');el.querySelector('strong').textContent=FLOORS[game.floor-1];el.classList.add('show');setTimeout(()=>el.classList.remove('show'),1800);}
...
function updateAim(){const aiming=renderer.mode==='grenade';aimBanner.hidden=!aiming;
  $('#grenade-label').textContent=aiming?'確認投擲':`手榴彈 ${game.player.grenades}`;
  $('[data-action="grenade"]').classList.toggle('aiming',aiming);
  if(aiming)aimBanner.innerHTML=`<span>${distance(renderer.aim,game.player)<=2?'⚠ 你也在爆炸範圍！':'點地板或方向鍵選落點'}<br><small>半徑 2 格 · 再按「確認投擲」</small></span><button data-action="cancelAim" aria-label="取消投擲">✕</button>`;
}
function grenade(){if(renderer.mode==='grenade'){act('grenade',renderer.aim);return;}if(game.player.grenades<=0){notify('手榴彈已用盡。');return;}renderer.mode='grenade';const e=game.targeted;renderer.aim=e&&distance(e,game.player)<=5?{x:e.x,y:e.y}:{x:game.player.x,y:game.player.y};updateAim();}
...
function close(){if(game.pendingPerks||game.status!=='playing')return;$('#modal').close();$('#battle').focus({preventScroll:true});}
function modalAction(type,arg){close();act(type,arg);}

function showInventory() {
  const p=game.player,ground=game.groundWeapon;
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><h2>帶走你需要的。</h2>
    <div class="pack-resources"><span>✚ 醫療 <b>${p.meds}</b></span><span>◉ 手榴彈 <b>${p.grenades}</b></span><span>◇ 廢料 <b>${p.scrap}</b></span></div>
    <p class="ammo-summary">子彈 ${p.reserve} · 電池 ${p.energy} · 榴彈 ${p.ordnance}<br>換裝、拆解、改裝與補給各消耗 1 回合。查看背包不耗時。</p>
    ...
    ${ground?`<div class="ground-loot"><strong>附近軍械：${WEAPONS[ground.weapon].name}</strong><button data-take="${ground.weapon}" ${p.owned.length>=PACK_LIMIT||p.owned.includes(ground.weapon)?'disabled':''}>拾取武器</button><small>${p.owned.length>=PACK_LIMIT?'武器欄已滿；拆解一把後，再次打開背包拾取。':'拾取後不會自動換裝。'}</small></div>`:''}
...
function showHelp(){modal(`<div class="eyebrow">FIELD MANUAL / BUILD 3.0</div><h2>每一步，都要算數。</h2><p>穿越六層設施。第 3 層擊敗封鎖官，第 6 層摧毀核心守衛，從綠色電梯撤離。</p><div class="help-grid"><b>四方向</b><span>↑ ↓ ← → 就是畫面上的上下左右。方向鈕、鍵盤 WASD / 方向鍵或點相鄰格移動。</span><b>射擊</b><span>點敌人、掩體或油桶鎖定，再按開火 / Space。目標鈕 / Tab 輪換敵人。</span><b>掩體</b><span>牆角可探身互射，也能提供掩護。箱體與牆角均降低命中率並減傷；爆炸無視箱體。</span><b>命中率</b><span>暴露且靜止 97%；移動 −22%。箱體 −35%、牆角 −42%，敵我規則相同。鎖定欄顯示當前命中率。</span><b>手榴彈</b><span>按 G 或手榴彈，點地板選落點，再確認投擲。射程 5、爆炸半徑 2，會自傷和連鎖引爆。</span><b>背包</b><span>B 開啟背包，可帶 3 把武器。Q 換槍、R 裝填、H 醫療、F 防禦減伤、句點等待。</span><b>補給</b><span>走上道具即可拾取。靠近終端以廢料交易。武器可拆解回收、改裝增傷至 +3。</span><b>危險</b><span>! 代表敵人蓄勢。紅色轟炸格兩回合後爆炸；綠色毒液與橘色高熱格會傷害站在上面的單位。</span><b>撤離</b><span>到綠色電梯鄰格，按電梯按鈕或 E。頭目未死時電梯鎖定。</span><b>存檔</b><span>每步自動儲存在目前瀏覽器。設定可匯出 / 匯入存檔，避免換裝置失去進度。</span></div><button class="modal-button" data-modal="close">收到，返回戰場 →</button>`,true);}
function settings(){modal(`<div class="eyebrow">SYSTEM / BUILD 3.0</div><h2>作戰設定</h2><p>任務 ${game.seed} · 第 ${game.floor} 層 · ${game.turn} 回合<br>${storage.available?'進度已自動儲存。':'本機儲存不可用，請匯出存檔保留進度。'}</p><div class="modal-row"><button class="modal-button secondary" data-modal="sound">音效：${audio.enabled?'開啟':'關閉'}</button><button class="modal-button secondary" data-modal="help">作戰指南</button></div><div class="modal-row"><button class="modal-button secondary" data-modal="export">匯出存檔</button><button class="modal-button secondary" data-modal="import">匯入存檔</button></div><button class="modal-button secondary" data-modal="journal">任務紀錄與敵人圖鑑</button><button class="modal-button secondary" data-modal="restart">重新部署新任務</button><button class="modal-button" data-modal="close">繼續任務 →</button>`);}
function showResult(){const won=game.status==='won',p=game.player;modal(`<div class="eyebrow">${won?'SIGNAL RESTORED':'SIGNAL LOST'} / RUN ${game.seed}</div><h2>${won?'灰燼之中，仍有回音。':'這次的訊號，到此為止。'}</h2><p>${won?'核心已沉默。你帶著設施的秘密，搭上最後一班撤離電梯。':'你留下的紀錄將協助下一位行動員。掩體、補給與適時撤退，都能改變下一次任務。'}</p><div class="result-stats"><div><b>${pad(game.floor)}</b>抵達樓層</div><div><b>${p.kills}</b>消滅敵人</div><div><b>${game.turn}</b>行動回合</div></div><p>總傷害 ${p.stats.damage} · 投擲 ${p.stats.grenades} · 資料 ${p.lore.length}/6</p><button class="modal-button" data-modal="new">重新部署 →</button>`);}
function newGame(seed){game=new Game(seed);renderer.game=game;renderer.camera={x:game.player.x,y:game.player.y};renderer.effects=[];cancelAim();lastStatus='playing';previousFloor=game.floor;$('#modal').close();update();floorToast();}
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.modal){switch(b.dataset.modal){
    case 'close':close();break;case 'bag':showInventory();break;case 'journal':showJournal();break;case 'bestiary':bestiary();break;case 'help':showHelp();break;
    ...
    case 'restart':modal('<div class="eyebrow">REDEPLOY</div><h2>重新部署？</h2><p>目前任務將結束。可先在設定匯出存檔。</p><label class="seed-field">地圖種子（留空隨機）<input id="new-seed" type="number" min="0" max="999999999" placeholder="例：2026" inputmode="numeric"></label><button class="modal-button" data-modal="new">確認，开始新任務</button><button class="modal-button secondary" data-modal="close">返回目前任務</button>');break;
  ...
  switch(b.dataset.action){
    case 'game':$('#battle').focus();break;case 'help':showHelp();break;case 'settings':settings();break;case 'bag':showInventory();break;case 'terminal':showTerminal();break;
    ...
    case 'center':renderer.camera={x:game.player.x,y:game.player.y};break;
    default:act(b.dataset.action);
  }
});
$('#modal').addEventListener('cancel',e=>{if(game.pendingPerks||game.status!=='playing')e.preventDefault();});
...
document.addEventListener('keydown',e=>{
  ...
  else if(actions[key]){e.preventDefault();act(actions[key]);}
...
window.addEventListener('pagehide',()=>saveGame(game));update();floorToast();
