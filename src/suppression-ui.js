...
export function suppressionHelp(){
 return `壓制最多 ${T.max} 層，每層射擊與近戰命中 −${T.accuracy}；${T.pinned} 層以上不能移動（仍可攻擊、裝填、使用道具），每回合結束層數減半。一次攻擊打出至少 ${T.weaponRounds} 發並命中，目標得 1 層；壓制射擊消耗 ${T.skillRounds} 發、每發命中 −${T.skillAccuracy}，範圍內敵人保底 1 層，命中者再加 1 層。頭目少 1 層，機械不受壓制。壓制抗性每階使單次壓制合計 −1 層，最高 3 階，可用學習資料提升；部分職業、重裝兵與頭目天生 1 階。未識別貨櫃可能掉出武器或學習資料：在背包道具分頁使用或拆解（+${LEARNING_SCRAP} 廢料），都不耗回合；學會的主動技能與職業技能共用一格預備欄。`;
}
