...
test('feeding invariants 4,5: passive growth consumes no fuel, turret pays per shot and empty stomach bites',()=>{
 const g=arena(),a=g.allies[0],b=g.player.petBond;grow(g,'turret',T.thresholds.turret[5]);grow(g,'armor',T.thresholds.armor[4]);b.fuel=petFuelCost(g.player,'shot')*2;const e=foe(g);g.turn=2;sure(g);const reserve=g.player.reserve;allyAct(g,a);assert.equal(g.effects.filter(e=>e.weaponId==='pet_turret').length,2);assert.equal(b.fuel,0);assert.ok(e.hp<200);assert.equal(g.player.reserve,reserve);assert.equal(petWeapon(g.player).melee,true);const hp=e.hp;Object.assign(e,{x:12});allyAct(g,a);assert.ok(e.hp<hp);assert.equal(b.fuel,0);
});
test('turret prefers adjacent bite; insufficient fuel permits one shot; a killed target does not cost a second shot',()=>{
 for(const kill of [false,true]){const g=arena(),a=g.allies[0],b=g.player.petBond;grow(g,'turret',T.thresholds.turret[5]);b.fuel=petFuelCost(g.player,'shot')*(kill?2:1);const e=foe(g);if(kill)e.hp=1;g.turn=2;sure(g);allyAct(g,a);assert.equal(g.effects.filter(e=>e.weaponId==='pet_turret').length,1);assert.equal(b.fuel,kill?petFuelCost(g.player,'shot'):0);}
 const g=arena(),a=g.allies[0];grow(g,'turret',T.thresholds.turret[5]);g.player.petBond.fuel=500;foe(g,12);g.turn=2;sure(g);allyAct(g,a);assert.equal(g.player.petBond.fuel,500);
});
test('feeding invariant 6: output is timed, costs fuel, waits without backlog, falls under pet even at carry cap',()=>{
 const g=arena(),b=g.player.petBond,a=g.allies[0];grow(g,'extrusion',1);b.fuel=petFuelCost(g.player,'output');g.player.grenades=4;for(let i=0;i<T.outputIntervals[0]-1;i++)g.action('wait');assert.equal(g.items.length,0);g.action('wait');assert.equal(g.items.length,1);assert.equal(g.items[0].type,'grenade');assert.equal(g.items[0].x,a.x);assert.equal(b.fuel,0);for(let i=0;i<T.outputIntervals[0]+1;i++)g.action('wait');assert.equal(g.items.length,1);assert.equal(b.outputRemaining,0);b.fuel=petFuelCost(g.player,'output');g.action('wait');assert.equal(g.items[0].amount,2);assert.equal(b.outputRemaining,T.outputIntervals[0]);
});
test('feeding invariant 7: growth caps reject more food, final extrusion selects freely without ticking',()=>{
 const g=arena();for(const [line,n] of Object.entries(Object.fromEntries(Object.entries(T.thresholds).map(([k,v])=>[k,v.at(-1)]))))grow(g,line,n);for(const q of [{optionId:'life'},{optionId:'plates'},{optionId:'weapon',weaponSlot:1},{optionId:'grenade:frag'}]){assert.equal(g.action('feedPet',q),false);assert.equal(g.turn,1);}const before=g.player.petBond.outputRemaining;assert.ok(g.action('setPetOutput',{kind:'emp'}));assert.equal(g.turn,1);assert.equal(g.player.petBond.outputRemaining,before);assert.equal(petFeedingState(g).output.kind,'emp');assert.ok(Game.restore(g.serialize()));
});
...
test('feeding invariants 9,12,13: life floor, no healing from growth, pure medical healing and cardinal adjacency',()=>{
 const g=arena(),a=g.allies[0];a.hp=10;g.player.petBond.growth.vitality=45;assert.ok(feed(g,'life'));assert.equal(a.maxHp,150);assert.equal(a.hp,10);assert.equal(g.player.petBond.growth.vitality,50);const growth=structuredClone(g.player.petBond.growth);assert.ok(feed(g,'medkit'));assert.equal(a.hp,85);assert.deepEqual(g.player.petBond.growth,growth);
...
test('v35 rejects malformed and missing bond values, legacy states, mismatched identities and overflow',()=>{
 const g=arena();for(const mutate of [d=>delete d.player.petBond,d=>d.player.petBond.fuel=-1,d=>d.player.petBond.fuel=petCapacity(g.player)+1,d=>d.player.petBond.growth.armor=T.thresholds.armor.at(-1)+1,d=>d.player.petBond.actorId='ally-2',d=>d.allies[0].status='packed',d=>d.player.petBond.reviveRemaining=9,d=>d.player.petBond.outputChoice='invalid',d=>d.player.petBond.fedThrowables.frag=1]){const raw=JSON.parse(g.serialize());mutate(raw.data);assert.equal(Game.restore(JSON.stringify(raw)),null);}
});
...
test('vitality lethal guard is once per actual floor; armor growth permits cover and only direct damage gets extra reduction',()=>{
 const g=arena(),a=g.allies[0];grow(g,'vitality',T.thresholds.vitality[3]);grow(g,'armor',T.thresholds.armor[4]);assert.equal(activeTrait(a,'no_cover'),false);g.damageAlly(a,999);assert.equal(a.hp,1);assert.equal(a.status,'active');g.damageAlly(a,999);assert.equal(a.status,'reforming');for(let i=0;i<8;i++)g.action('wait');g.damageAlly(a,999);assert.equal(a.status,'reforming');assert.deepEqual(g.player.petBond.tenacityUsedFloors,[1]);
 const h=arena(),b=h.allies[0];grow(h,'armor',T.thresholds.armor[4]);h.damageAlly(b,24,null,true);assert.equal(b.hp,74);b.hp=90;h.damageAlly(b,24,null,true,true);assert.equal(b.hp,70);
});
test('full stomach permits only fitting ammo; quoting is side-effect-free and combat regeneration never costs fuel',()=>{
 const g=arena(),b=g.player.petBond,a=g.allies[0];b.fuel=petCapacity(g.player)-ammoFuel('pistol');const before=g.serialize(),q=petFeedQuote(g,{optionId:'ammo:pistol'});assert.equal(q.amount,1);assert.equal(g.serialize(),before);feed(g,'ammo:pistol');assert.equal(b.fuel,petCapacity(g.player));assert.equal(feed(g,'ammo:pistol'),false);grow(g,'vitality',50);a.hp=20;b.lastCombatTurn=g.turn;g.action('wait');assert.equal(a.hp,25);assert.equal(b.fuel,petCapacity(g.player));
});

test('a committed turret shot still spends fuel if its ray disappears; ranged fire records corner exposure',()=>{
 const g=arena(),a=g.allies[0],b=g.player.petBond;grow(g,'turret',T.thresholds.turret[2]);b.fuel=petFuelCost(g.player,'shot');foe(g);g.turn=2;let checks=0;g.shotClear=()=>++checks===1;sure(g);allyAct(g,a);assert.equal(b.fuel,0);assert.equal(g.effects.find(e=>e.weaponId==='pet_turret').miss,true);
 const h=arena(),pet=h.allies[0];grow(h,'turret',T.thresholds.turret[2]);h.player.petBond.fuel=500;foe(h);h.turn=2;let exposed=0;const original=h.recordExposure.bind(h);h.recordExposure=(...args)=>{exposed++;return original(...args);};sure(h);allyAct(h,pet);assert.equal(exposed,1);
});
