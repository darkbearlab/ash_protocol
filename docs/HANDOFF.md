...
3.165.0：氣泡雜訊由 `signal-glitch.js` 的 `bubbleGlitch`（每個泡泡自己排程）與 `drawGlitchedBox`（長方形版的切片與殘影）畫；`renderer.drawCallouts` 的每個泡泡改成一個用 `this.ctx` 的閉包，才能被導到暫存畫布。射程黃框是 `renderer.flashRange`／`drawRangeFlash`（`RANGE_FLASH` 170／110 毫秒），由控制器在拒絕代碼為 `out_of_range` 時呼叫。坑洞欄杆以 id 前綴 `edge-pit-` 辨認（`barrier-art.js` 的 `pitRail`、`PIT_RAIL_HEIGHT`）。自動改鎖是控制器的 `retarget()`（本機設定 `ash-auto-retarget`），只改 `game.target`，和目標鈕同一種免費操作。
