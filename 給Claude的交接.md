...
使用者請 Claude 接手已採用音訊的遊戲整合。先讀 [音訊接入交接](docs/AUDIO_INTEGRATION_HANDOFF.md)，再按 [採用總表](docs/AUDIO.md) 取素材。衝鋒槍單發已補完；遊戲尚未接入。音訊提交在本機 main，尚未推送，其他 worktree 勿假設遠端已有。此項不更改後續其他工作分工。
...
- `AGENTS.md` 是專案入口；不熟悉模組／保存責任時看 `docs/HANDOFF.md` 的相關段落。
- 規劃時看願望清單；實作時由 `docs/DESIGN.md` 找對應規格。歷史取捨有疑問才查 CHANGELOG／archive。
- 驗收或接續工作時，讀對應版本與 SHA 的驗證紙條／報告，分開實測和未測結果。
- 分工疑問看本檔授權表；發布看 `docs/RELEASE.md`。沿用本次對話已讀且未更動的內容。
