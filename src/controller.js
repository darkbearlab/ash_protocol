...
function showWorkshop(message=''){
  ...
  const row=i=>{const unit=p.productionLines[i];
    ...
    return `<div class="ground-loot"><strong>序列 ${i+1} · ${def.name}${unit.payload?` · ${GRENADES[unit.payload].name}`:''}${Number.isInteger(unit.weapon)?` · ${game.weaponAt(unit.weapon).name}`:''}</strong><small>${def.text}${unit.payload?'':Number.isInteger(unit.weapon)?`彈匣 ${p.ammo[unit.weapon]}/${game.weaponAt(unit.weapon).mag}，部署時從你身上補滿。`:'部署時從你身上裝填手槍彈。'}</small><button data-workshop-deploy="${i}" ${reason?'disabled':''}>${reason||'部署 · 選位置 · 1 回合'}</button></div>`;};
