import {ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon} from './allies.js';
...
$('#battle').addEventListener('pointerup',e=>{
  ...
  const ally=game.localAllies.find(a=>distance(a,pos)===0);if(ally){notify(`${allyName(ally)} · ${ally.status==='down'?'倒地，靠近以技能救援':ally.status==='destroyed'?'已毀':`HP ${ally.hp}/${ally.maxHp}${ally.kind==='drone'?` · 彈藥 ${ally.ammo}/${allyWeapon(ally).mag}`:''}`}。`);return;}
