...
http.createServer(async (req,res) => {
  try {
    ...
    const publicFile = pathname === '/' || ['/index.html','/material-review.html','/style.css','/expansion.css','/manifest.webmanifest','/sw.js'].includes(pathname) || /^\/(src\/[a-z0-9-]+\.js|assets\/(?:pixel\/(?:(?:terrain-v1|walls-v1|portraits|classes-v1|scenery-v1|doors-v1|nests-v1|killhouse-v1)\/)?)?[a-z0-9-]+\.(svg|png|json))$/.test(pathname);
