...
export const MAP_STYLES={facility:{themes:facilityThemes,materials:null,selection:MATERIAL_SELECTION},killhouse:{themes:{industrial:{atlas:KILLHOUSE_ATLAS,tile:32,columns:4,roles:{floor:0,cover:7}}},materials:{floor:0,face:1,cap:2,doorClosed:3,doorBroken:4,doorFrame:5,doorCap:6,cover:7}}};
export const mapStyle=g=>Object.hasOwn(MAP_STYLES,g?.mapStyle)?g.mapStyle:'facility';
export function styleSprite(style,role){const def=MAP_STYLES[typeof style==='string'?style:mapStyle(style)],index=def?.materials?.[role];return Number.isInteger(index)?{url:def.themes.industrial.atlas,x:index%4*32,y:Math.floor(index/4)*32,size:32}:null;}
export const mapStyleAtlases=()=>[...new Set(Object.values(MAP_STYLES).flatMap(s=>Object.values(s.themes).map(t=>t.atlas)))];
