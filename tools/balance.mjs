...
export function play(seed,maxActions=1800) {
  ...
  for(let i=0;i<maxActions&&g.status==='playing';i++) {
    ...
    if(targets.length&&p.ammo[p.weapon]>0){g.target=targets[0].id;act('fire');continue;}
    ...
    const needs=g.items.filter(item=>{const type=itemAmmo(item.type),weapons=p.owned.map(i=>g.weaponAt(i)).filter(w=>w.ammoType===type);return (type&&weapons.length&&p[AMMUNITION[type].key]<Math.max(...weapons.map(w=>w.mag))*2)||(item.type==='med'&&p.meds<1)||(item.type==='grenade'&&p.grenades<1);}).sort((a,b)=>distance(p,a)-distance(p,b));
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){const count=Number(process.argv[2]||24);const rows=Array.from({length:count},(_,i)=>play(i+1));console.table(rows);console.log(JSON.stringify({wins:rows.filter(r=>r.status==='won').length,total:count,stuck:rows.filter(r=>r.status==='playing').length,invalid:rows.reduce((n,r)=>n+r.invalid,0)}));}
