...
| `tools/pixelize_comms_portraits.py` | 管制員臉譜（3.170.0）：GPT Image 原圖 → 量格線取像素中心、合併相似色、壓進 Mega Drive 色票、16 色、64×64，輸出到 art/comms-v1/<代號>/；`--install` 複製到 assets/pixel/comms-v1，遊戲裡的檔案被手動改過就拒絕覆蓋（`--force` 才蓋，紀錄在 art/comms-v1/installed.json）。流程見 art/comms-v1/README.md |
| `server.mjs` | 本機靜態伺服器 |
