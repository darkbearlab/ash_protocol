...
| `src/i18n.js`、`src/text-zh-tw.js` | 語言表（3.166.0）：`t(id,{slot})` 讀整句樣板、`{n|one|other}` 單複數、`sentence()` 補句號；中文表依模組分組。新句子一律進表，不在程式裡拼字；規則不准比對中文字串（tests/i18n.test.mjs）。見 TEXT_INVENTORY.md 第九節 |
| `src/main.js` | 瀏覽器入口 |
...
- 3.166.0 起給玩家看的句子寫成語言表裡的整句樣板（`t()`，src/i18n.js），主詞可能是「你」的句子分兩個代號；判斷結果用代碼或 `t()` 的回傳值比對，不讀中文字。改字後跑 `node qa/text-identity.mjs --compare <基準>` 證明其他句子沒變。
