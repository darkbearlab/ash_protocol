...
export const SCENERY_FURNITURE={
  desk:{name:t('sceneryFurniture.desk.name'),hp:55,sprite:12},pallet:{name:t('sceneryFurniture.pallet.name'),hp:75,sprite:13},
  ...Object.fromEntries(['rover','shuttle'].flatMap((type,v)=>Array.from({length:6},(_,i)=>[`${type}_${i}`,{name:t(`sceneryFurniture.${type}.${i===2||i===3?'cockpit':'part'}`),hp:i===2||i===3?75:55,sprite:Math.floor(i/2)*4+i%2+v*2,full:i===2||i===3}]))),
};
const vehicles=type=>Array.from({length:6},(_,i)=>({style:`${type}_${i}`,x:1+i%2,y:1+Math.floor(i/2)}));
export const LARGE_MODULE_TYPES={
  office:{name:t('largeModuleTypes.office.name'),code:'OFFICE',color:'#9ebfc1',size:[4,4],parts:[{style:'desk',x:0,y:1},{style:'desk',x:3,y:1}],edges:[['partition',1,0,2,0],['partition',1,1,2,1],['low_partition',0,1,0,2],['low_partition',3,1,3,2]]},
  warehouse:{name:t('largeModuleTypes.warehouse.name'),code:'CARGO',color:'#c1b293',size:[4,4],parts:[{style:'pallet',x:0,y:0},{style:'pallet',x:3,y:0},{style:'pallet',x:0,y:3},{style:'pallet',x:3,y:3}],edges:[]},
  garage:{name:t('largeModuleTypes.garage.name'),code:'GARAGE',color:'#b8bf9e',size:[4,5],parts:vehicles('rover'),edges:[]},
  hangar:{name:t('largeModuleTypes.hangar.name'),code:'HANGAR',color:'#a0b8cf',size:[4,5],parts:vehicles('shuttle'),edges:[]},
};
