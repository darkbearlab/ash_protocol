...
test('new fields roundtrip through full backup; legacy v11 receives tags but preserves its campaign',()=>{
  ...
  const full=JSON.parse(arena().serialize());full.version=11;full.data.player.traits=Array.from({length:64},(_,i)=>({id:'fast',source:`old:${i}`}));const preserved=Game.restore(JSON.stringify(full));assert.ok(preserved);assert.equal(preserved.player.traits.length,64+1+arena().player.traits.length,'3.158.0: the v11 tag keeps its own source; the class passives are re-derived under character:soldier');assert.ok(activeTrait(preserved.player,'biological'));
