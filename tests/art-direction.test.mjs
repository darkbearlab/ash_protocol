...
test('review page is standalone and never uses campaign/profile storage; it uses the shared renderer and exports validated JSON',async()=>{
  // 3.197.0: a development page in tools/, never published or cached for players.
  const body=await readFile(new URL('../tools/material-review.js',import.meta.url),'utf8'),html=await readFile(new URL('../tools/material-review.html',import.meta.url),'utf8');assert.ok(body.includes('drawWall(c,g'));assert.ok(body.includes('validateSelection(draft)'));assert.ok(body.includes("a.download='selection.json'"));assert.ok(!/localStorage|sessionStorage|ash-save|ash-profile/.test(body));
  for(const id of ['catalog','scene','floor','face','cap','tone','download','load','reset','import','status'])assert.ok(html.includes('id="'+id+'"'));const sw=await readFile(new URL('../sw.js',import.meta.url),'utf8');for(const file of ['art-tone.js','materials.js','material-selection.js'])assert.ok(sw.includes('./src/'+file));assert.ok(!sw.includes('material-review'),'the review page is not shipped');
  const build=await readFile(new URL('../tools/build.mjs',import.meta.url),'utf8');assert.ok(!build.includes('material-review'));assert.ok(build.includes('README\\.md$/i.test(source)'),'no README ships');
});
