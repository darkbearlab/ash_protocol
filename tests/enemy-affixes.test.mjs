...
test('7/10: resistance subtracts once after summed sources, innate rank and repeatable manual max three, machines immune',()=>{
 ...
 const robot=makeEnemy('drone',1,1,'r');applySuppression(robot,100);assert.equal(robot.suppression,0);assert.equal(UNKNOWN_LOOT.length,25);   // 3.135.0: −night vision −infrared +goggles; 3.136.0: +5 melee weapons +carry
});
