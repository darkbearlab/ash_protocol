...
function update(view=renderer.game) {
  ...
  $('[data-action="reload"] strong').textContent=`${view.actionCost('reload')===0?'快填':'裝填'} ${p.ammo[p.weapon]}/${w.mag}`;
  $('[data-action="reload"]').title=`裝填：${view.actionCost('reload')} 回合`;
  $('#quick-weapon').textContent=`${w.code} · ${p.ammo[p.weapon]} / ${reserve}`;$('#quick-weapon').title=`${AMMUNITION[w.ammoType].name}：備彈 ${reserve}/${view.ammoCapacity(w.ammoType)}`;
  ...
  $('#status-effects').textContent=[exposed?`暴露 ${exposed}`:threats.length?'掩護':view.cover.length?'牆 / 箱旁':'',p.moved?'移動':'',threats.some(e=>view.accuracy(e,p).sidePenalty)?`側身 ${threats.filter(e=>view.accuracy(e,p).sidePenalty).length}`:'',p.guard?'減傷 50%':'',p.plates?`護甲板 ${p.plates}`:'',p.focus?'瞄準 +15':'',p.evasive?'閃避 +15':'',p.poison?`中毒 ${p.poison}`:'',initiative(p)<0?'快速':initiative(p)>0?'緩速':''].filter(Boolean).join(' · ');
  ...
  for(const b of document.querySelectorAll('.control-deck button'))b.disabled=Boolean(playback)||view.status!=='playing'||(Object.hasOwn(PREPARED_CATEGORIES,b.dataset.action)&&(!preparedEntry(p,b.dataset.action)||(preparedEntry(p,b.dataset.action).resource&&p[preparedEntry(p,b.dataset.action).resource]<=0)));
  updateAim(view);
  if(playback)return;
...
function interact(){if(game.status!=='playing'){showResult();return;}if(renderer.mode==='grenade'){act('usePrepared',{category:'grenade',target:renderer.aim});return;}
  ...
  if(options[0]?.action==='bag')showInventory('weapon');else if(options[0]?.action==='terminal')showTerminal();else if(options[0]?.action==='descend')act('interact');
}
function grenade(){if(renderer.mode==='grenade'){cancelAim();return;}const entry=preparedEntry(game.player,'grenade');if(!entry){notify('請先在背包預備手榴彈。');return;}if(game.player[entry.resource]<=0){notify(`${entry.name}已用盡。`);return;}renderer.mode='grenade';const e=game.targeted;renderer.aim=e&&distance(e,game.player)<=5?{x:e.x,y:e.y}:{x:game.player.x,y:game.player.y};updateAim();}
...
function showIntro(){modal(`<div class="eyebrow">ASH PROTOCOL / 灰燼協定</div><h2>深入寂靜。<br>活著回來。</h2><p>木星邊境的厄瑞玻斯設施停止回傳訊號。你是最後一名接近它的行動員。</p><p>深入六層設施，擊敗封鎖官與核心守衛，帶著紀錄撤離。只有你採取行動，敵人才會行動。</p><p>協定點數 ${profile().protocol.balance} · 探索資料、擊敗頭目與完成樓層可累積，死亡仍保留。現在可用於永久攜行升級。</p><div class="intro-rules"><span>↑ 四方向探索</span><span>⌖ 掩體與命中率</span><span>◷ 等待觀察</span><span>▣ 搜刮與換裝</span></div><p>${resumable||entered?`${characterName(game.player.character)} · 任務 ${game.seed} · 第 ${game.floor} 層 · ${game.turn} 回合`:'新任務可選 Soldier 士兵或 Recon 偵察兵，兩者免費。'}</p><button class="modal-button" data-modal="${game.status==='playing'&&(resumable||entered)?'enter':'deploy'}">${game.status!=='playing'?'開始新任務':resumable||entered?'繼續任務':'開始行動'} →</button><button class="modal-button secondary" data-modal="carrying">永久攜行升級</button><button class="modal-button secondary" data-modal="help">閱讀作戰指南</button><button class="modal-button secondary" data-modal="settings">備份與設定</button>`);}
function showDeployment(){
  const live=game.status==='playing'&&(entered||resumable);
  modal(`<div class="eyebrow">DEPLOYMENT</div><h2>選擇行動員</h2><p>${live?'確認後會放棄目前任務，保留已賺點數與永久進度。':'兩名基礎角色皆免費。角色只在新任務開始時選擇。'}</p>
    <fieldset class="character-choices"><legend>角色</legend>${Object.entries(CHARACTERS).map(([id,c])=>`<label class="character-choice"><input type="radio" name="character" value="${id}" ${id===game.player.character?'checked':''}><span><strong>${c.name} · ${c.label}</strong><small>${c.text}</small><small>生命 100 · ${c.weapons.map(slot=>WEAPONS[slot].name).join('／')}</small>${c.traits.map(t=>`<small><b>${TRAITS[t].name}</b>：${TRAITS[t].text}</small>`).join('')}</span></label>`).join('')}</fieldset>
...
`;
    ...
    content=`<p>目前預備：<strong>${selected?.name||'無'}</strong>。預備／取消預備不耗回合；回到戰場後按對應按鈕使用。</p>
      <div class="prepared-list">${options.map(([id,entry])=>`<article class="prepared-entry ${p.prepared[category]===id?'equipped':''}"><h3>${entry.icon} ${entry.name}</h3><p>${entry.text}${id==='medkit'?` 目前回復 ${45+p.healBonus} 生命。`:''}</p><span>${entry.resource?`持有 ${p[entry.resource]}${category==='grenade'?` / ${game.ammoCapacity('grenade')}`:''}`:'已學會'}</span><button class="modal-button secondary" data-prepare-category="${category}" data-prepare-id="${id}" ${p.prepared[category]===id?'disabled':''}>${p.prepared[category]===id?'已預備':'預備 · 不耗回合'}</button></article>`).join('')}</div>
  ...
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><h2>作戰背包</h2><p>${characterName(p.character)}</p>
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.context){close();if(b.dataset.context==='bag')showInventory('weapon');else if(b.dataset.context==='terminal')showTerminal();else act('interact');return;}
  ...
  if(b.dataset.modal){switch(b.dataset.modal){
    ...
    case 'new':{const value=$('#new-seed')?.value,seed=value!==undefined&&value!==''?Number(value):undefined;if(seed!==undefined&&(!Number.isInteger(seed)||seed<0||seed>999999999)){notify('種子需為 0–999999999 的整數。');return;}newGame(seed,$('input[name="character"]:checked')?.value);break;}
  }return;}
