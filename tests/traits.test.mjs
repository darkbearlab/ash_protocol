...
function arena(Type=Game){const g=new Type(310);clearGeneratedMap(g);g.barriers=[];g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));Object.assign(g.player,{x:10,y:10,plates:0});g.enemies=[];g.props=[];g.items=[];g.hazards=[];g.marks=[];g.rng=Object.assign(()=>0,{state:()=>0});/* Cancel Soldier innate +8 to isolate this rule. */g.player.combatModifiers={rangedAccuracy:-8};g.reveal();return g;}
