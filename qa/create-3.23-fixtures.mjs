...
function arena(){const g=new Game(323);g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));g.lighting=g.grid.map(row=>row.map(()=>1));g.props=[];g.barriers=[];g.enemies=[];g.items=[];g.hazards=[];g.smoke=[];g.marks=[];g.rooms=[];g.traces=[];Object.assign(g.player,{x:10,y:10,hp:999,maxHp:999,traits:[],armor:0,plates:0});g.end={x:20,y:20};return g;}
