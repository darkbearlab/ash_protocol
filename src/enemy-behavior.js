...
export const ENEMY_WEAPONS={rifleman:{rounds:1},raider:{rounds:2},gunner:{rounds:1},sniper:{rounds:1}};
export function enemyWeapon(e){const rapid=rapidFireModifiers(e);return {range:ENEMY_TYPES[e.type]?.range||1,...rapid,rounds:(ENEMY_WEAPONS[e.type]?.rounds||1)+rapid.extraRounds};}
...
function attack(ctx){const {g,e,p,def}=ctx;const fired=def.range>1,rapid=fired&&activeTrait(e,'rapid_fire'),weapon=enemyWeapon(e),rounds=fired?weapon.rounds:1,hits=new Set();let firedRounds=0;
 if(rapid)revealEnemyAffix(g,e,'suppressor');
 if(fired&&lightingEffects(g,{...e,traits:e.traits.filter(t=>t.id!=='night_vision')},p).penalty>0)revealEnemyAffix(g,e,'night_vision');
 ...
 for(let n=0;n<rounds&&p.hp>0;n++){const before=p.hp,roundDamage=Math.max(1,Math.floor(totalDamage/baseRounds)+(n%baseRounds<totalDamage%baseRounds?1:0));firedRounds++;
        ...
        if(unitTree(e).fixedTile&&e.aim&&!g.shotClear(e,e.aim)){const edge=firstBarrierOnRay(g.barriers,e,e.aim);g.log('狙擊彈被門或隔板阻擋。');g.effects.push({type:'enemyShot',attackerType:e.type,from:{x:e.x,y:e.y},to:edge?{x:edge.x,y:edge.y}:{...e.aim},damage:0});if(edge)g.damageProp(edge,roundDamage);}
          ...
          petCombat(g,p);const chance=def.range>1?g.accuracy(e,p).chance:g.meleeAccuracy(e,p);
