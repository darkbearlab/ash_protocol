...
test('recipe maps through floor 60 satisfy all eight topology invariants with three distinct target rooms',()=>{
  ...
  for(const seed of [1,2,42])for(let floor=1;floor<=60;floor++){
    const map=generateWithRecipes(seed,floor),accessible=reachable(map,map.start);
    ...
    const solids=new Set(map.props.filter(p=>p.hp>0&&['cover','barrel','nest'].includes(p.type)).map(key));
    ...
    if(floor%10===0)assert.deepEqual(generateWithRecipes(seed,floor),map);
  }
...
test('custom descriptors and saved damage survive archive, backup and return without consulting the recipe pool',()=>{
  ...
  const first=Object.fromEntries(fields.map(k=>[k,structuredClone(g[k])]));Object.assign(g.player,g.exitPoint);assert.ok(g.descend());assert.equal(g.generation.version,8);
  ...
  g.generation.base.recipe.id='removed-custom-recipe';g.generation.base.recipeId='removed-custom-recipe';assert.ok(Game.restore(g.serialize()));
  ...
  Object.assign(copy.player,copy.exitPoint);assert.ok(copy.descend());for(const [k,v]of Object.entries(first))assert.deepEqual(copy[k],v);assert.deepEqual(copy.mapGenerations,[6,8]);
});

test('custom save validation rejects false layouts, endpoints, paths and slot ownership',()=>{
  const g=new Game(1);assert.equal(g.generation.version,8);
  for(const change of [d=>d.generation.base.recipe.layout[0][0]='E',d=>d.generation.recipeId='other',d=>d.rooms[0].cellIds=[8],d=>d.startRoom=d.rooms.find(r=>r.cellIds.length>1).id,d=>d.openings[0].path[0]={x:0,y:0},d=>d.slots[0].roomId=999,d=>d.annexes=[{type:'dock'}]]){
    const raw=JSON.parse(g.serialize());change(raw.data);assert.equal(Game.restore(JSON.stringify(raw)),null);
  }
  assert.equal(themeAt({...g,props:[]},g.rooms[0]),g.generation.base.recipe.theme);
