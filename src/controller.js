...
<fieldset class="term-list"><legend>SELECT CONTRACT</legend>${Object.entries(MISSIONS).map(([id,m])=>`<div class="term-row"><label class="term-pick"><input type="radio" name="mission" value="${id}" ${id===game.mission.id?'checked':''}><span class="term-caret" aria-hidden="true">&gt;</span><span class="term-body"><span class="term-name">${m.name}</span><span class="term-meta">${(m.depth||6)}F</span></span></label><p class="term-note">${m.text}</p></div>`).join('')}</fieldset>
