...
function update(view=renderer.game) {
  ...
  for(const category of Object.keys(PREPARED_CATEGORIES)){
    ...
    if(category!=='grenade')button.querySelector('strong').textContent=entry?`${entry.short}${category==='skill'?' '+skillStatus(p,p.prepared.skill):count===null?'':` ${count}`}`:`${PREPARED_CATEGORIES[category]}未預備`;
  ...
  for(const b of document.querySelectorAll('.control-deck button'))b.disabled=Boolean(playback)||view.status!=='playing'||(b.dataset.action==='skill'&&(!canUseSkill(p,p.prepared.skill)||p.control.disabled))||(b.dataset.action==='reload'&&w.melee)||(Object.hasOwn(PREPARED_CATEGORIES,b.dataset.action)&&(!preparedEntry(p,b.dataset.action)||(preparedEntry(p,b.dataset.action).resource&&p[preparedEntry(p,b.dataset.action).resource]<=0)));
