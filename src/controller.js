...
import {TERMINAL_TUNING,TERMINAL_PACK,terminalCost,terminalDeal,terminalRemaining,tradeHoldings,offerReason} from './terminal.js';
...
function setAim(pos){const launch=renderer.mode==='launch',range=launch?game.weapon.range:5;if(distance(pos,game.player)<=range&&game.grid[pos.y]?.[pos.x]===1&&game.visible(pos)){renderer.aim=pos;updateAim();}else notify(launch?`落點需在視線內 ${range} 格以內。`:'投擲落點需在視線內 5 格以內。');}
...
function showInventory(tab=inventoryTab,message='') {
  ...
  if(category==='weapon')content=`    <div class="pack-ammo">${AMMO_IDS.map(id=>`<span style="--tint:${AMMUNITION[id].tint};--tint-bg:${AMMUNITION[id].tint}26" title="${AMMUNITION[id].name}"><i>${AMMUNITION[id].short}</i><b>${p[AMMUNITION[id].key]}<small>/${game.ammoCapacity(id)}</small></b></span>`).join('')}</div>
    <div class="pack-weapons">${p.owned.map(index=>{const w=game.weaponAt(index),d=game.weaponDamage(index),active=index===p.weapon,level=p.upgrades[index];const tint=w.ammoType?AMMUNITION[w.ammoType].tint:MELEE_TINT;return `<article class="pack-weapon ${active?'equipped':''}" style="--tint:${tint};--tint-bg:${tint}1f"><div><div class="pack-weapon-head"><h3>${w.name}${level?` +${level}`:''}</h3><small>${active?'已裝備':'備用'} · ${w.code}</small></div><span>傷害 ${d.min}–${d.max}${w.closeRange?` · 1–2 格 ${game.weaponDamage(index,{x:p.x+1,y:p.y}).min}–${game.weaponDamage(index,{x:p.x+1,y:p.y}).max}／命中 +15`:''}${w.burst?` ×${w.burst}${w.burstRange!==undefined?'／遠距 ×1':''}`:''} · 射程 ${w.range} · ${ammoName(w)} · ${magazineLabel(w,p.ammo[index])}${w.locked?' · 固定裝備':''}</span>${p.affixes[index]?`<p>${w.affixText}</p>`:''}<div class="pack-actions"><button data-equip="${index}" ${active?'disabled':''}>裝備 · ${game.actionCost('weapon',index)} 回合</button><button data-salvage="${index}" ${p.owned.length<=1||w.locked?'disabled':''}>${w.locked?'固定裝備':`拆解 +${salvageValue(p,index)}`}</button></div></div></article>`;}).join('')}</div>
...
function terminalOfferGroups(){
  const p=game.player;
  return [
    {name:'醫療',rows:[{id:'heal',title:'醫療修復',detail:`回復 ${healingAmount(p,TERMINAL_TUNING.healAmount)} 生命並清除中毒。`},{id:'med',title:'醫療包 +1',detail:`持有 ${p.meds}，不設上限。`}]},
    ...
    {name:'道具',rows:Object.entries(TERMINAL_ITEMS).map(([id,offer])=>{const entry=PREPARED_CATALOG.item[id],held=offer.wear?p.wearables.includes(offer.wear):p[offer.resource];return {id,title:entry.name,detail:offer.wear?(held?'已經有一件。':'佩戴型：戴上與脫下各 1 回合。'):`持有 ${held}，不設上限。`};})},
