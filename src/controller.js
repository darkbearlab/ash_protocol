...
import {Game,WEAPONS,FLOORS,floorInfo,PERKS,ENEMY_TYPES,enemyName,distance,protocolSettlement,itemUseReason,deployCoverReason,TERMINAL_ITEMS,TERMINAL_STOCK,terminalReason} from './engine.js';
...
function act(type,arg) {
  ...
  if(success||(type!=='grenade'&&type!=='deployCover'&&!(type==='usePrepared'&&arg?.category==='grenade')))cancelAim();update();return success;
}
function move(dx,dy){if(renderer.mode==='deploy'){act('deployCover',[dx,dy]);return;}if(renderer.mode==='pet'){setPetAim({x:renderer.aim.x+dx,y:renderer.aim.y+dy});return;}if(renderer.mode==='drone'){setDroneAim({x:renderer.aim.x+dx,y:renderer.aim.y+dy});return;}if(renderer.mode==='grenade'){const pos={x:renderer.aim.x+dx,y:renderer.aim.y+dy};setAim(pos);}else if(renderer.mode==='suppress'){setSuppressAim({x:renderer.aim.x+dx,y:renderer.aim.y+dy});}else act('move',[dx,dy]);}
...
function updateAim(view=renderer.game){const deploying=renderer.mode==='deploy',commanding=renderer.mode==='pet'||renderer.mode==='drone',aiming=renderer.mode==='grenade',suppressing=renderer.mode==='suppress',preview=suppressing&&renderer.aim?suppressivePreview(view,renderer.aim):null,b=$('#interact'),options=interactions(view);
  ...
  b.disabled=Boolean(playback)||(view.status==='playing'&&!aiming&&!commanding&&!suppressing&&!deploying&&!options.length)||Boolean(preview?.reason);
  b.querySelector('strong').textContent=view.status!=='playing'?'結果':deploying?'取消設置':commanding?(renderer.mode==='drone'?'確認部署':'確認指揮'):aiming?'確認投擲':suppressing?`確認壓制 · ${preview?.rounds??0} 發`:options.length>1?'互動':options[0]?.label||'互動';
  b.classList.toggle('aiming',aiming||commanding||suppressing||deploying);$('[data-action="skill"]')?.classList.toggle('aiming',suppressing);b.title=view.allyTravelSummary||'';
...
`;
    ...
    const status=(id,entry)=>entry.wear?(p.prepared[category]===id?'脫下 1 回合':'戴上 1 回合'):entry.resource?`×${p[entry.resource]}`:category==='skill'?(ALLY_SKILLS.includes(id)?allySkillState(game,id):`${skillStatus(p,id)}${p.skillState[id]?.cooldown?` · 冷卻 ${p.skillState[id].cooldown}`:''}`):'已學會';
...
function showTerminal(){const p=game.player;modal(`<div class="eyebrow">SUPPLY TERMINAL / ONE USE</div><h2>選擇需要的補給。</h2><p>此終端只能交易一次，耗費 1 回合。持有廢料：${p.scrap}。容量不足時，多出的補給留在腳下。</p><button class="perk" data-terminal="heal" ${terminalReason(game,'heal')?'disabled':''}><strong>醫療修復 · 15 廢料</strong><span>回復 ${healingAmount(p,60)} 生命並清除中毒。</span></button>${Object.entries(TERMINAL_ITEMS).map(([id,offer])=>{const entry=PREPARED_CATALOG.item[id],held=offer.wear?p.wearables.includes(offer.wear):p[offer.resource];return `<button class="perk" data-terminal="${id}" ${terminalReason(game,id)?'disabled':''}><strong>${entry.name} · ${offer.cost} 廢料</strong><span>${offer.wear?(held?'已經有一件。':'佩戴型：戴上與脫下各 1 回合。'):`持有 ${held} / ${TERMINAL_STOCK}。`}</span></button>`;}).join('')}${AMMO_IDS.map(id=>{const offer=TERMINAL_AMMO[id],info=AMMUNITION[id];return `<button class="perk" data-terminal="${id}" ${terminalReason(game,id)?'disabled':''}><strong>${info.name} +${offer.amount} · ${offer.cost} 廢料</strong><span>目前 ${p[info.key]} / ${game.ammoCapacity(id)}</span></button>`;}).join('')}${Object.entries(GRENADES).map(([id,entry])=>`<button class="perk" data-terminal="${entry.item}" ${terminalReason(game,entry.item)?'disabled':''}><strong>${entry.name} +${entry.amount} · ${entry.cost} 廢料</strong><span>持有 ${p[entry.resource]} · 共用容量 ${grenadeTotal(p)} / ${game.ammoCapacity('grenade')}</span></button>`).join('')}<button class="modal-button secondary" data-modal="close">返回戰場</button>`);}
