...
import {UNIT_BLUEPRINTS,buildReason,deployReason,deployedUnits,mountableSlots} from './workshop.js';
import {ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon,droneCells,defaultDroneCell,deployLimit,lineLimit,MUNITION_TUNING,TETHER,CARRY_DISTANCE,SUMMON_LIMIT,SUMMON_INTERVAL,SUMMON_TETHER,RALLY_TURNS,PET_TETHER,DRONE_HP,SENTRY_ARMOR,ENEMY_UNIT_TUNING} from './allies.js';
...
function showWorkshop(message=''){
  ...
  const row=i=>{const unit=p.productionLines[i];
    ...
    return `<div class="ground-loot"><strong>序列 ${i+1} · ${def.name}${unit.payload?` · ${GRENADES[unit.payload].name}`:''}${Number.isInteger(unit.weapon)?` · ${game.weaponAt(unit.weapon).name}`:''}</strong><small>${def.text}${unit.payload||unit.blueprint==='unit_bomber'?'':Number.isInteger(unit.weapon)?`彈匣 ${p.ammo[unit.weapon]}/${game.weaponAt(unit.weapon).mag}，部署時從你身上補滿。`:unit.blueprint==='unit_drone'?'部署時從你身上裝填能量電池。':'部署時從你身上裝填手槍彈。'}</small><button data-workshop-deploy="${i}" ${reason?'disabled':''}>${reason||'部署 · 選位置 · 1 回合'}</button></div>`;};
  modal(`<div class="eyebrow">WORKSHOP / ${p.productionLines.length} OF ${lines}</div><h2>工坊</h2><p>部署中 ${deployedUnits(game).length}/${deployLimit(p)} · 廢料 ${p.scrap}<br>敵方藍圖：${p.blueprints.map(id=>UNIT_BLUEPRINTS[id].name).join('、')||'尚未取得'}<br>部署出去的機體不會回收；留在別層的不算部署上限。</p>${message?`<p role="status">${escapeHTML(message)}</p>`:''}${Array.from({length:lines},(_,i)=>row(i)).join('')}<button class="modal-button" data-modal="close">返回戰場 →</button>`,true);
}
function blueprintOption(id,def,payload=null,weapon=null){
  ...
  return `<button class="perk" data-workshop-blueprint="${id}"${payload?` data-payload="${payload}"`:''}${gun?` data-weapon="${weapon}"`:''} ${reason?'disabled':''}><strong>${def.name}${throwable?` · ${throwable.name}`:gun?` · ${gun.name}`:''} · ${def.cost} 廢料${throwable?'＋1 顆':''} · 1 回合</strong><span>${def.text}<br>${detail}${reason?`<br>${reason}`:''}</span></button>`;
}
