...
export const SKILLS={
 suppressive_fire:{name:t('skills.suppressive_fire.name'),short:t('skills.suppressive_fire.short'),icon:'◇',action:'suppressiveFire',cost:1,duration:0,cooldown:0,target:'area',text:t('skills.suppressive_fire.text')},
 anchor:{name:t('skills.anchor.name'),short:t('skills.anchor.short'),icon:'◇',action:'skill',toggle:true,cost:1,duration:1,cooldown:0,text:t('skills.anchor.text')},
 workshop:{name:t('skills.workshop.name'),short:t('skills.workshop.short'),icon:'▣',action:'skill',cost:0,duration:0,cooldown:0,card:t('skills.workshop.card'),text:t('skills.workshop',{repairCost:REPAIR_TUNING.cost,droneHp:DRONE_HP,droneMag:FOLLOW.mag,droneRange:FOLLOW.range,sentryArmor:SENTRY_ARMOR,sentryMag:SENTRY.mag,carry:CARRY_DISTANCE,tether:TETHER,munitionHp:MUNITION_TUNING.hp,droneUnitHp:ENEMY_UNIT_TUNING.drone.hp,droneUnitRange:ENEMY_UNIT_TUNING.drone.range,bomberHp:ENEMY_UNIT_TUNING.bomber.hp,wardenHp:ENEMY_UNIT_TUNING.warden.hp,bossHp:ENEMY_UNIT_TUNING.boss.hp,lines:WORKSHOP_TUNING.lines,deploy:WORKSHOP_TUNING.deploy})},
 ...
 early_warning:{name:t('skills.early_warning.name'),short:t('skills.early_warning.short'),icon:'⌖',action:'skill',cost:0,duration:1,cooldown:5,radius:8,text:SKILL_TEXTS.early_warning({radius:8,cooldown:5,mark:markValues(null)})},signal_break:{name:t('skills.signal_break.name'),short:t('skills.signal_break.short'),icon:'⌁',action:'skill',cost:0,duration:3,cooldown:6,text:SKILL_TEXTS.signal_break({duration:3,cooldown:6})},
 grapple:{name:t('skills.grapple.name'),short:t('skills.grapple.short'),icon:'◇',action:'skill',cost:1,duration:0,cooldown:GRAPPLE_COOLDOWN,text:t('skills.grapple',{range:GRAPPLE_RANGE,cooldown:GRAPPLE_COOLDOWN})},
 camouflage:{name:t('skills.camouflage.name'),short:t('skills.camouflage.short'),icon:'◇',action:'skill',cost:0,duration:CAMO_DURATION,cooldown:CAMO_COOLDOWN,cooldownAfterEffect:true,text:SKILL_TEXTS.camouflage({duration:CAMO_DURATION,cooldown:CAMO_COOLDOWN})},
};
...
export function skillStatus(player,id){const state=player.skillState?.[id];if(SKILLS[id]?.toggle)return t(state?.remaining?'skills.status.release':'skills.status.activate');return state?.remaining?t('skills.status.active',{n:state.remaining}):state?.cooldown?t('skills.status.cooldown',{n:state.cooldown}):t('skills.status.ready');}
