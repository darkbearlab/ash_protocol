...
function showCarrying(message=''){
  ...
  const grid=cards=>`<div class="upgrade-grid">${cards.join('')}</div>`;
  const sec=label=>`<div class="eyebrow settings-section">${label}</div>`;
  ...
  const operators=Object.values(CHARACTERS).map(c=>shown(c.label,c.skills?.length?SKILLS[c.skills[0]].name:`生命 ${c.hp} · 裝甲 ${c.armor}`));
  ...
  const skills=Object.values(SKILLS).map(k=>shown(k.name,`${k.cost?`耗 ${k.cost} 回合`:'免費啟動'}${k.cooldown?` · 冷卻 ${k.cooldown}`:''}`));

  modal(`<div class="eyebrow">PERMANENT EQUIPMENT</div><h2>永久升級</h2>
${message?`<p role="status">${escapeHTML(message)}</p>`:''}
<p>協定點數 <strong>${p.protocol.balance}</strong>${spent?` · 已投入 ${spent}`:''}</p>
${sec('攜行容量')}<p>每種獨立三階，費用 20／40／70 點。容量含目前兵種加成。</p>${grid(carry)}
${sec('幹員')}${grid(operators)}
${sec('武器')}${grid(guns)}
${sec('投擲物')}${grid(throwables)}
${sec('技能')}${grid(skills)}
${sec('重置')}<p>${spent?`清除全部攜行等級並退還已投入的 ${spent} 點。`:'目前沒有已投入的攜行升級。'}重置免費，不影響解鎖、任務與紀錄。</p>
