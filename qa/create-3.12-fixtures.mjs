...
function arena(name,character='soldier'){const g=new Game(3120,[],0,character);g.runId=`qa-character-${name}-${Date.now()}`;g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));Object.assign(g.player,{x:10,y:10,hp:200,maxHp:200});g.enemies=[];g.items=[];g.props=[];g.hazards=[];g.marks=[];g.rooms=[];g.end={x:20,y:20};return g;}
