...
3.141.0：霰彈槍的錐形改成彈丸（`src/shotgun.js` 的 `pelletsAt`、`pelletChance`；`Game.pelletDamage`；`hitTarget` 的第六個參數 `pellets` 讓裝甲逐顆扣、掩體改用武器的 `pelletCover`）。一般單發的傷害算式順序刻意沒變，`pellets` 為空時結果逐位元相同。電漿步槍 52–66、穿透 1（`weaponStats` 的穿透上限因此放寬到 1）。限定掉落詞條在 `AFFIXES` 帶 `dropOnly`，`rollAffix` 用另一個雜湊先判定它們，其餘擲法不變；3.141.1 起詞條可帶 `notOn`（追獵不上霰彈槍），抽到的武器改用 `${seed}:repick` 雜湊在其餘詞條裡重選；`affixAllowed` 在讀檔時擋掉掛錯武器的詞條。`shotCost` 為每次射擊的耗彈，開火與機器人都要看它，不能只看「彈匣 > 0」。貫穿的路徑在 `src/lance.js`。
