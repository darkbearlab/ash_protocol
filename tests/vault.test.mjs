...
import {VAULT_TUNING,VAULT_LOOT,vaultCapable} from '../src/vault.js';
...
test('saves carry the vault and the card, and refuse forged ones',()=>{
 ...
 assert.equal(forge(d=>{d.barriers.find(b=>!b.vault).indestructible=true;}),null,'an ordinary door made unbreakable');
 ...
 for(const b of legacy.data.barriers)if(b.vault){delete b.vault;delete b.locked;delete b.indestructible;}
