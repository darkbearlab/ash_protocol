...
export function buildReason(g,blueprint,payload){
...
export function buildUnit(g,blueprint,payload){
 const reason=buildReason(g,blueprint,payload);if(reason)return g.fail(reason);
 const p=g.player,def=UNIT_BLUEPRINTS[blueprint];p.scrap-=def.cost;
 if(def.payload){p[GRENADES[payload].resource]--;p.productionLines.push({blueprint,payload});}else p.productionLines.push({blueprint});
 g.log(`消耗 ${def.cost} 廢料${def.payload?`與 1 顆${GRENADES[payload].name}`:''}，${def.name}已放進生產序列。`);return true;
}
...
export function deployUnit(g,line,point=null){
 ...
 if(unit.payload)a.payload=unit.payload;else reloadDrone(g,a);
...
const validLine=u=>{
 ...
 return UNIT_BLUEPRINTS[u.blueprint].payload?keys==='blueprint,payload'&&validPayload(u.payload):keys==='blueprint';
};
