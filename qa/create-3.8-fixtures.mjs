...
function arena(){const g=new Game(380);g.runId=`qa-weapons-${Date.now()}`;g.grid=Array.from({length:SIZE},(_,y)=>Array.from({length:SIZE},(_,x)=>Number(x>0&&y>0&&x<SIZE-1&&y<SIZE-1)));Object.assign(g.player,{x:10,y:10,scrap:200});g.enemies=[];g.items=[];g.props=[];g.hazards=[];g.marks=[];g.rooms=[];g.end={x:20,y:20};return g;}
