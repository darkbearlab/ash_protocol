...
function showIntro(){modal(`<div class="eyebrow">ASH PROTOCOL / 灰燼協定</div><h2>深入寂靜。<br>活著回來。</h2><p>木星邊境的厄瑞玻斯設施停止回傳訊號。你是最後一名接近它的行動員。</p><p>深入六層設施，擊敗封鎖官與核心守衛，帶著紀錄撤離。只有你採取行動，敵人才會行動。</p><p>協定點數 ${profile().protocol.balance} · 探索資料、擊敗頭目與完成樓層可累積，死亡仍保留。現在可用於永久攜行升級。</p><div class="intro-rules"><span>↑ 四方向探索</span><span>⌖ 掩體與命中率</span><span>◷ 等待觀察</span><span>▣ 搜刮與換裝</span></div><div class="operator-identity">${resumable||entered?portraitMarkup(game.player.portrait,game.status):''}<p>${resumable||entered?`${characterName(game.player.character)} · 任務 ${game.seed} · 第 ${game.floor} 層 · ${game.turn} 回合`:'新任務可選 Soldier 士兵或 Recon 偵察兵，兩者免費。'}</p></div><button class="modal-button" data-modal="${game.status==='playing'&&(resumable||entered)?'enter':'deploy'}">${game.status!=='playing'?'開始新任務':resumable||entered?'繼續任務':'開始行動'} →</button><button class="modal-button secondary" data-modal="carrying">永久攜行升級</button><button class="modal-button secondary" data-modal="help">閱讀作戰指南</button><button class="modal-button secondary" data-modal="settings">備份與設定</button>`);}
function showDeployment(){
  const live=game.status==='playing'&&(entered||resumable);
  deploymentFaces=deploymentPortraits(Object.keys(CHARACTERS));
  modal(`<div class="eyebrow">DEPLOYMENT</div><h2>選擇行動員</h2><p>${live?'確認後會放棄目前任務，保留已賺點數與永久進度。':'兩名基礎角色皆免費。角色只在新任務開始時選擇，頭像隨機配發並隨本局保存。'}</p>
    <fieldset class="character-choices"><legend>角色</legend>${Object.entries(CHARACTERS).map(([id,c])=>`<label class="character-choice"><input type="radio" name="character" value="${id}" ${id===game.player.character?'checked':''}><span class="character-copy"><span class="operator-identity">${portraitMarkup(deploymentFaces[id])}<strong>${c.name} · ${c.label}</strong></span><small>${c.text}</small><small>生命 100 · ${c.weapons.map(slot=>WEAPONS[slot].name).join('／')}</small>${c.traits.map(t=>`<small><b>${TRAITS[t].name}</b>：${TRAITS[t].text}</small>`).join('')}</span></label>`).join('')}</fieldset>
...
`;
  ...
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><div class="operator-identity">${portraitMarkup(p.portrait)}<div><h2>作戰背包</h2><p>${characterName(p.character)}</p></div></div>
...
function newGame(seed,character){const portrait=deploymentFaces[character];if(!validCharacter(character)||!validPortrait(portrait)){notify('請選擇有效角色。');return;}if(game.status==='playing'&&(entered||resumable)){try{abandonRun(game);}catch(error){backupError(error);return;}}entered=true;resumable=false;game=new Game(seed,profile().unlocks.weapons,profile().upgrades.carrying,character,portrait);playback=null;renderer.game=game;renderer.camera={x:game.player.x,y:game.player.y};renderer.effects=[];cancelAim();lastStatus='playing';previousFloor=game.floor;$('#modal').close();update();floorToast();}
