...
export function decoyReason(g,pos){
 ...
 if(!floorTile(g,pos))return t('field-gear.decoyPickFloor');
...
export function throwDecoy(g,pos){
 ...
 g.log(fooled.length?t('field-gear.decoyFooled',{n:fooled.length}):t('field-gear.decoyNoTakers'));
 return true;
}
...
export function damageDecoy(g,amount){
 ...
 if(d.hp<=0){g.decoy=null;g.log(t('field-gear.decoyDestroyed'),true);}
}
...
export function tickDecoy(g){if(g.decoy&&g.turn>=g.decoy.expires){g.decoy=null;g.log(t('field-gear.decoyExpired'));}}
...
export function mineReason(g,pos){
 const p=g.player;
 if(!(p.mines>0))return t('field-gear.noMine');
 if((g.mines||[]).length>=MINE_TUNING.max)return t('field-gear.mineCap',{n:MINE_TUNING.max});
 if(!floorTile(g,pos))return t('field-gear.minePickFloor');
 if(distance(p,pos)>MINE_TUNING.range||!g.visible(pos))return t('field-gear.mineRange',{range:MINE_TUNING.range});
 if(standing(g,pos)||g.solid(pos.x,pos.y)||g.props.some(o=>o.x===pos.x&&o.y===pos.y)||mineAt(g,pos.x,pos.y))return t('field-gear.mineOccupied');
 if(g.exitPoint&&g.exitPoint.x===pos.x&&g.exitPoint.y===pos.y)return t('field-gear.mineExit');
 return '';
}
...
export function detonateMine(g,m){
 ...
 g.mines=g.mines.filter(o=>o!==m);g.log(t('field-gear.mineBlast'),true);
...
export const exoReason=p=>activeTrait(p,'large')?t('field-gear.exoTooLarge'):'';
...
export function breakExo(g){
 ...
 g.log(t('field-gear.exoBroken'),true);
}
