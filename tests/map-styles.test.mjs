...
import {MAP_STYLES,KILLHOUSE_ATLAS,KILLHOUSE_PROPS_ATLAS,styleSprite} from '../src/map-styles.js';
...
test('kill house has one tile per role, same floor for every room theme, separate walls and door states',()=>{
 ...
 assert.equal(themes.themeAt(g,{x:1,y:1}),'industrial');assert.equal(Object.keys(MAP_STYLES.killhouse.materials).length,36);
 for(const role of Object.keys(MAP_STYLES.killhouse.materials))assert.ok([KILLHOUSE_ATLAS,KILLHOUSE_PROPS_ATLAS].includes(styleSprite(g,role).url));
