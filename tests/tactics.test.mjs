...
function arena(seed=7){const g=new Game(seed);g.barriers=[];g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));g.lighting=g.grid.map(row=>row.map(()=>1));g.player.x=10;g.player.y=10;g.props=[];g.items=[];g.hazards=[];g.marks=[];g.enemies=[];g.reveal();return g;}
