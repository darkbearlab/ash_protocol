...
test('runtime generation is deterministic, conserves normal enemies/supplies, excludes objectives and preserves v1',()=>{
 for(const seed of [1,2,3])for(const floor of Array.from({length:60},(_,i)=>i+1)){
  /* 3.137.0: the skeleton keeps classic hit points, so compare against a classic run */const g=generate(seed,floor,[],{curve:'classic',offset:0}),old=generateWithRecipes(seed,floor);assert.deepEqual(g,generate(seed,floor,[],{curve:'classic',offset:0}));assert.ok(generationSafe(g));assert.ok(validMapMetadata(g));assert.deepEqual(g.enemies.filter(e=>!e.expendable).map(({affixes,elite,traits,keycard,...e})=>({...e,traits:traits.filter(t=>!t.source.startsWith('affix:'))})),old.enemies);assert.deepEqual(allSupplies({...g,props:g.props.filter(p=>p.kind!=='unknown'&&p.kind!=='vault')}),allSupplies(old));/* 3.146.0: the vault's one item is added on top */assert.ok(g.props.filter(p=>p.kind==='unknown').every(p=>p.contents.length===1));assert.ok(g.enemies.filter(e=>e.hp>0).length<=RUNTIME_TUNING.liveLimit);assert.ok(g.enemies.filter(e=>e.expendable).every(e=>!eligibleMissionEnemy(e)));assert.equal(new Set(g.enemies.map(key)).size,g.enemies.length);
 }
