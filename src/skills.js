...
import {TETHER,CARRY_DISTANCE,SUMMON_LIMIT,SUMMON_INTERVAL,SUMMON_TETHER,RALLY_TURNS,PET_TETHER,DRONE_HP,SENTRY_ARMOR,WORKSHOP_TUNING,allyWeapon} from './allies.js';
...
export const SKILLS={
 ...
 workshop:{name:'工坊',short:'工坊',icon:'▣',action:'skill',cost:0,duration:0,cooldown:0,card:'打開生產序列 · 生產與部署各 1 回合',text:`打開生產序列，不耗回合。空的序列花廢料生產機體（1 回合）；完成的機體點身邊 2 步內空格部署（1 回合），部署後不回收。追隨無人機 ${DRONE_HP} HP、${FOLLOW.mag} 發、射程 ${FOLLOW.range}，跟在你身邊、可以換位；定點砲台裝甲 ${SENTRY_ARMOR}、可用掩體、${SENTRY.mag} 發、不移動。機體在你 ${CARRY_DISTANCE} 步內自己用你的手槍彈換彈，繩索 ${TETHER} 格。基礎 ${WORKSHOP_TUNING.lines} 條序列、同時部署 ${WORKSHOP_TUNING.deploy} 台，職業升級各可加到 4。`},
