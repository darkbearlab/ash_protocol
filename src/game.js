...
export class Game {
  ...
  hurt(e,damage,attacker=null) {
    ...
    const beforeHp=e.hp;e.hp-=damage;injuryCallout(this,e,beforeHp);this.player.stats.damage+=damage;if(damage>0)addTrace(this,e,activeTrait(e,'mechanical')?'oil':'blood');
  ...
  environmentTurn() {
    ...
    else if(p.poison>0){p.poison--;const damage=Math.max(0,SWARM_TUNING.poisonDamage-p.hazmat);p.hp-=damage;if(damage)this.log(`中毒傷害 −${damage}，剩餘 ${p.poison} 回合。`,true,'中毒造成傷害。');}
  ...
  static restore(raw) {
    try {
      ...
      if(!validRuntime(g)||!validSwarm(g))return null;
