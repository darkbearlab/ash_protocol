import {DRONE_REPAIR_COST,DRONE_REPAIR_FRACTION,droneRepairReason,ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon} from './allies.js';
...
function update(view=renderer.game) {
  ...
  for(const category of Object.keys(PREPARED_CATEGORIES)){
    ...
    button.title=entry?`${entry.name}：${entry.text}${category==='skill'?' 目前'+skillStatus(p,p.prepared.skill)+'；冷卻剩餘 '+(p.skillState[p.prepared.skill]?.cooldown||0)+'。':''}`:`到背包預備${PREPARED_CATEGORIES[category]}`;
    button.setAttribute('aria-label',entry?`使用預備${PREPARED_CATEGORIES[category]}：${entry.name}${category==='skill'?'，'+skillStatus(p,p.prepared.skill):''}${count===null?'':`，剩餘 ${count}`}`:`${PREPARED_CATEGORIES[category]}未預備`);
  }
...
function move(dx,dy){if(renderer.mode==='pet'){setPetAim({x:renderer.aim.x+dx,y:renderer.aim.y+dy});return;}if(renderer.mode==='grenade'){const pos={x:renderer.aim.x+dx,y:renderer.aim.y+dy};setAim(pos);}else act('move',[dx,dy]);}
...
function updateAim(view=renderer.game){const commanding=renderer.mode==='pet',aiming=renderer.mode==='grenade',b=$('#interact'),options=interactions(view);
  ...
  b.querySelector('strong').textContent=view.status!=='playing'?'結果':commanding?'確認指揮':aiming?'確認投擲':options.length>1?'互動':options[0]?.label||'互動';
  b.classList.toggle('aiming',aiming||commanding);b.title=view.allyTravelSummary||'';
}
function interact(){if(renderer.mode==='pet'){act('commandPet',renderer.aim);return;}if(game.status!=='playing'){showResult();return;}if(renderer.mode==='grenade'){act('usePrepared',{category:'grenade',target:renderer.aim});return;}
...
function skill(){if(renderer.mode==='pet'){cancelAim();return;}if(game.player.prepared.skill==='pet_command'&&game.activeAllies.some(a=>a.kind==='pet')){renderer.mode='pet';renderer.aim={x:game.player.x,y:game.player.y};notify('點目的地後按右下確認；點自己代表召回。');updateAim();}else act('usePrepared',{category:'skill'});}
...
function showDeployment(){
  ...
  modal(`<div class="eyebrow">DEPLOYMENT</div><h2>選擇行動員</h2><p>${live?'確認後會放棄目前任務，保留已賺點數與永久進度。':'三名基礎角色皆免費。角色只在新任務開始時選擇，頭像隨機配發並隨本局保存。'}</p>
    <fieldset class="character-choices"><legend>角色</legend>${Object.entries(CHARACTERS).map(([id,c])=>`<label class="character-choice"><input type="radio" name="character" value="${id}" ${id===game.player.character?'checked':''}><span class="character-copy"><span class="operator-identity">${portraitMarkup(deploymentFaces[id])}<strong>${c.name} · ${c.label}</strong></span><small>${c.text}</small><small>生命 ${c.hp||100} · 裝甲 ${c.armor||0} · 護甲板 ${c.plates||0} · ${c.weapons.map(slot=>WEAPONS[slot].name).join('／')}</small><small>${combatStatSummary({character:id})} · 基礎投擲容量 ${capacity('grenade')+classCarryBonus(id,'grenade')}</small><small>醫療包 ×${startingSupplies(id).meds} · ${Object.values(GRENADES).filter(g=>startingSupplies(id)[g.resource]>0).map(g=>g.short+' ×'+startingSupplies(id)[g.resource]).join('／')}</small>${(c.skills||[]).map(id=>`<small><b>${SKILLS[id].name}</b>：${SKILLS[id].text}</small>`).join('')}${c.traits.map(t=>`<small><b>${TRAITS[t].name}</b>：${TRAITS[t].text}</small>`).join('')}</span></label>`).join('')}</fieldset>
    <fieldset class="character-choices mission-choices"><legend>任務 · 六層深入／三層往返</legend>${Object.entries(MISSIONS).map(([id,m])=>`<label class="character-choice"><input type="radio" name="mission" value="${id}" ${id===game.mission.id?'checked':''}><span><strong>${m.name}</strong><small>${m.text}</small></span></label>`).join('')}</fieldset><p>第 3 層仍需擊敗封鎖官。三層往返取件後回入口上樓，回程不補給；六層非核心任務可迴避核心守衛。指定殲滅目標以青色菱形標記。回收耗 1 回合，不占背包容量。</p>
...
function droneMaintenance(){
  ...
  if(a.status!=='packed')return `<p>${a.status==='destroyed'?'殘骸須相鄰以僚機技能回收':'請先回收機體'}，才能在此修復。</p>`;
...
`;
    ...
    content=`${category==='grenade'?`<p>投擲物共用容量：<strong>${grenadeTotal(p)} / ${game.ammoCapacity('grenade')}</strong>。全部種類合計，永久手榴彈攜行升級可提高此上限。使用消耗 1 回合。</p>`:''}<p>目前預備：<strong>${selected?.name||'無'}</strong>。預備／取消預備不耗回合；回到戰場後按對應按鈕使用。</p>
      <div class="prepared-list">${options.map(([id,entry])=>`<article class="prepared-entry ${p.prepared[category]===id?'equipped':''}"><h3>${entry.icon} ${entry.name}</h3><p>${entry.text}${id==='medkit'?` 目前回復 ${45+p.healBonus} 生命。`:''}</p><span>${entry.resource?`持有 ${p[entry.resource]}`:category==='skill'?skillStatus(p,id)+' · 冷卻剩餘 '+(p.skillState[id]?.cooldown||0):'已學會'}</span><button class="modal-button secondary" data-prepare-category="${category}" data-prepare-id="${id}" ${p.prepared[category]===id?'disabled':''}>${p.prepared[category]===id?'已預備':'預備 · 不耗回合'}</button></article>`).join('')}</div>
...
function showCarrying(message=''){
  const p=profile();modal(`<div class="eyebrow">PERMANENT EQUIPMENT</div><h2>各彈種攜行升級</h2>${message?`<p role="status">${escapeHTML(message)}</p>`:''}<p>協定點數 <strong>${p.protocol.balance}</strong>。每種獨立購買三階，費用依序 20／40／70 點，只提高選定彈種的容量；手榴彈為四種投擲物共用上限。永久保留，不耗回合、不補充彈藥。下列容量含目前兵種加成；換兵種時仍保留永久升級。</p>${Object.entries(AMMUNITION).map(([id,info])=>{const level=p.upgrades.carrying[id],cost=CARRY_COSTS[level];return `<div class="ground-loot carry-upgrade"><strong>${info.name} · ${level} / ${CARRY_COSTS.length} 階</strong><small>容量 ${capacity(id,level)+classCarryBonus(game.player.character,id)}${cost!==undefined?` → ${capacity(id,level+1)+classCarryBonus(game.player.character,id)}`:''}</small><button data-buy-carry="${id}" data-carry-level="${level}" ${cost===undefined||p.protocol.balance<cost||!storage.available||storage.recoveryPending?'disabled':''}>${cost===undefined?'已滿階':`升級 · ${cost} 點`}</button></div>`;}).join('')}<p>舊版整組升級已退還已花點數，各彈種從 0 階重新分配；任務保留，超額備彈留在腳下。彈匣不受影響。</p><button class="modal-button secondary" data-modal="close">返回</button>`);
}
...
$('#battle').addEventListener('pointerup',e=>{
  ...
  const ally=game.localAllies.find(a=>distance(a,pos)===0);if(ally){notify(`${allyName(ally)} · ${ally.status==='down'?'倒地，靠近以技能救援':ally.status==='destroyed'?'已毀，相鄰用僚機技能回收殘骸':`HP ${ally.hp}/${ally.maxHp}${ally.kind==='drone'?` · 彈藥 ${ally.ammo}/${allyWeapon(ally).mag}`:''}`}。`);return;}
