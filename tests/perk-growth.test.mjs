...
test('a new run levels at 2L+3 experience; a run from before 3.138.0 keeps L+2',()=>{
 const g=new Game(3);assert.equal(g.perkRules,3,'3.148.0: 升級 D');assert.deepEqual([1,2,5,19].map(l=>levelCost(g,l)),[5,7,13,41]);assert.equal(costToCap(g),437);
 ...
 const bad=JSON.parse(g.serialize());bad.data.perkRules=4;assert.equal(Game.restore(JSON.stringify(bad)),null);
 assert.equal(Game.restore(g.serialize()).perkRules,3);
});
...
test('the direct-number perks give less per rank; the utility perks are unchanged',()=>{
 ...
 const now=new Game(3),old=new Game(3);now.perkRules=2;old.perkRules=1;
