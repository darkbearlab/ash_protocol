...
import {UNIT_BLUEPRINTS,buildReason,deployReason,deployedUnits} from './workshop.js';
import {ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon,droneCells,defaultDroneCell,deployLimit,lineLimit,MUNITION_TUNING,TETHER,CARRY_DISTANCE,SUMMON_LIMIT,SUMMON_INTERVAL,SUMMON_TETHER,RALLY_TURNS,PET_TETHER,DRONE_HP,SENTRY_ARMOR} from './allies.js';
...
const notifyLatest=()=>{if(game.logs[0])notify(game.logs[0].text,{extra:lastActionLogs-1,danger:game.logs[0].danger});};
...
function skill(){if(renderer.mode==='pet'||renderer.mode==='drone'||renderer.mode==='suppress'){cancelAim();return;}const id=game.player.prepared.skill;if(id==='suppressive_fire'){startSuppressAim();return;}if(id==='pet_command'&&game.activeAllies.some(a=>a.kind==='pet')){renderer.mode='pet';renderer.aim={x:game.player.x,y:game.player.y};notify('點目的地後按右下確認；點自己代表召回。');updateAim();}else if(id==='workshop')showWorkshop();else act('usePrepared',{category:'skill'});}
...
${operatorColorPicker()}
<button class="modal-button" data-modal="deployDifficulty">下一步：選擇難度 →</button>
...
<button class="modal-button secondary" data-modal="deployBackOperator">← 返回</button>`,true);
}
...
function showWorkshop(message=''){
  ...
  const row=i=>{const unit=p.productionLines[i];
    ...
    return `<div class="ground-loot"><strong>序列 ${i+1} · ${def.name}${unit.payload?` · ${GRENADES[unit.payload].name}`:''}</strong><small>${def.text}${unit.payload?'':'部署時從你身上裝填手槍彈。'}</small><button data-workshop-deploy="${i}" ${reason?'disabled':''}>${reason||'部署 · 選位置 · 1 回合'}</button></div>`;};
  modal(`<div class="eyebrow">WORKSHOP / ${p.productionLines.length} OF ${lines}</div><h2>工坊</h2><p>部署中 ${deployedUnits(game).length}/${deployLimit(p)} · 廢料 ${p.scrap}<br>部署出去的機體不會回收；留在別層的不算部署上限。</p>${message?`<p role="status">${escapeHTML(message)}</p>`:''}${Array.from({length:lines},(_,i)=>row(i)).join('')}<button class="modal-button" data-modal="close">返回戰場 →</button>`,true);
}
function blueprintOption(id,def,payload=null){
  const p=game.player,reason=buildReason(game,id,payload??undefined),throwable=payload&&GRENADES[payload];
  return `<button class="perk" data-workshop-blueprint="${id}"${payload?` data-payload="${payload}"`:''} ${reason?'disabled':''}><strong>${def.name}${throwable?` · ${throwable.name}`:''} · ${def.cost} 廢料${throwable?'＋1 顆':''} · 1 回合</strong><span>${def.text}<br>${throwable?`投擲物：持有 ${p[throwable.resource]} 顆，製作時用掉 1 顆`:'武器：內建輕機槍（武器掛載之後開放）<br>彈藥：手槍彈，部署時從你身上裝填'}${reason?`<br>${reason}`:''}</span></button>`;
}
function showBlueprints(){
  const p=game.player;
  modal(`<div class="eyebrow">WORKSHOP / BLUEPRINTS</div><h2>選擇藍圖</h2><p>廢料 ${p.scrap} · 序列 ${p.productionLines.length}/${lineLimit(p)}</p>${Object.entries(UNIT_BLUEPRINTS).flatMap(([id,def])=>def.payload?Object.keys(GRENADES).map(payload=>blueprintOption(id,def,payload)):[blueprintOption(id,def)]).join('')}<button class="modal-button secondary" data-workshop>返回工坊</button>`,true);
}
...
function learningSection(){
  ...
  return `<h3>學習資料</h3>${entries.length?`<div class="learning-list">${entries.map(e=>`<article class="prepared-entry"><h3>${e.icon} ${e.title}<b>×${e.count}</b></h3><p>${e.detail}</p>${e.useReason?`<span>${e.useReason}</span>`:''}<div class="pack-actions"><button data-learn="${e.id}" ${e.useReason?'disabled':''}>使用 · 不耗回合</button><button data-dismantle-learning="${e.id}" ${e.dismantleReason?'disabled':''}>拆解 +${e.scrap} 廢料</button></div></article>`).join('')}</div>`:'<p class="pack-empty">沒有學習資料。開啟未識別貨櫃可能取得。</p>'}`;
}
...
function showInventory(tab=inventoryTab,message='') {
  ...
  let content='';
  if(category==='weapon')content=`    <div class="pack-ammo">${AMMO_IDS.map(id=>`<span style="--tint:${AMMUNITION[id].tint};--tint-bg:${AMMUNITION[id].tint}26" title="${AMMUNITION[id].name}"><i>${AMMUNITION[id].short}</i><b>${p[AMMUNITION[id].key]}<small>/${game.ammoCapacity(id)}</small></b></span>`).join('')}</div><p class="ammo-summary">備彈上限與彈匣分開計算。拾取只補到上限，多出的留在地上。<br>護甲板吸收直接傷害的至多一半，消耗後可拾取修復；不擋毒液或高熱。<br>換裝所需回合標於各武器按鈕；交換、拾取、拆解、改裝與補給各消耗 1 回合。比較不耗時；走上武器會在有空格時收下，相同槍種不再自動拆解。</p>
    <div class="pack-weapons">${p.owned.map(index=>{const w=game.weaponAt(index),d=game.weaponDamage(index),active=index===p.weapon,level=p.upgrades[index],cost=25+level*15;const tint=w.ammoType?AMMUNITION[w.ammoType].tint:MELEE_TINT;return `<article class="pack-weapon ${active?'equipped':''}" style="--tint:${tint};--tint-bg:${tint}1f"><div><h3>${w.name}${level?` +${level}`:''}<small>${active?'已裝備':'備用'} · ${w.code}</small></h3><p>${w.affixText}</p><span>傷害 ${d.min}–${d.max}${w.closeRange?` · 1–2 格 ${game.weaponDamage(index,{x:p.x+1,y:p.y}).min}–${game.weaponDamage(index,{x:p.x+1,y:p.y}).max}／命中 +15`:''}${w.burst?` ×${w.burst}${w.burstRange!==undefined?'／遠距 ×1':''}`:''} · 射程 ${w.range} · ${ammoName(w)} · ${magazineLabel(w,p.ammo[index])}${w.locked?' · 固定裝備':''}</span><div class="pack-actions"><button data-equip="${index}" ${active?'disabled':''}>裝備 · ${game.actionCost('weapon',index)} 回合</button>${active?`<button data-upgrade="${index}" ${p.scrap<cost||level>=3?'disabled':''}>${level>=3?'改裝已滿':`改裝 ${cost} 廢料`}</button>`:''}<button data-salvage="${index}" ${p.owned.length<=1||w.locked?'disabled':''}>${w.locked?'固定裝備':`拆解 +${20+level*10}`}</button></div></div></article>`;}).join('')}</div>
...
`;
    ...
    content=`${category==='grenade'?`<p>投擲物共用容量：<strong>${grenadeTotal(p)} / ${game.ammoCapacity('grenade')}</strong>。全部種類合計，上限為基礎值加職業加成。使用消耗 1 回合。</p>`:''}<p>目前預備：<strong>${selected?.name||'無'}</strong>。預備／取消預備不耗回合；回到戰場後按對應按鈕使用。</p>
      ...
      ${category==='item'?learningSection():''}${category==='skill'?`${petFeedingSection()}<h3>同行與留置友軍</h3><p>${game.allies.length?game.allies.map(a=>`${allyName(a)} · ${a.status==='reforming'?'消散，重生倒數中':a.status==='arriving'?'等候落點':a.status==='destroyed'?'已毀':'活動'} · 第 ${a.floor} 層 · HP ${a.hp}/${a.maxHp}${a.kind==='drone'?(a.payload?` · 裝填${GRENADES[a.payload].short}`:` · ${a.ammo}/${allyWeapon(a).mag} 發`):''}`).join('<br>'):'無'}。</p>${game.player.skills.includes('workshop')?`<p>工坊 · ${allySkillState(game,'workshop')}。預備工坊後按技能鈕打開。</p>`:''}<h3>目前被動規則</h3><p>${traitLabels(p).join(' · ')||'無'}。<br>${traitRuleLines(p).join('<br>')}</p><p>被動自動生效，不占主動技能預備欄。</p>`:''}`;
  }
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${game.weaponCapacity}</div><div class="operator-identity">${portraitMarkup(p.portrait)}<div><h2>作戰背包</h2><p>${characterName(p.character)} · 裝甲 ${p.armor}<br>${combatStatSummary(p)}${meleeSummary(p).map(line=>'<br>'+line).join('')}<br>${game.missionSummary}</p></div></div>
    ...
    ${runPerks()}<div class="inventory-tabs" role="tablist" aria-label="背包分類">${Object.entries(INVENTORY_TABS).map(([id,label])=>`<button id="pack-tab-${id}" role="tab" aria-controls="pack-panel" aria-selected="${id===category}" tabindex="${id===category?0:-1}" data-inventory-tab="${id}">${label}</button>`).join('')}</div>
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.workshopBlueprint){modalAction('buildUnit',{blueprint:b.dataset.workshopBlueprint,...(b.dataset.payload?{payload:b.dataset.payload}:{})});return;}
  ...
  switch(b.dataset.action){
    ...
    case 'item':act('usePrepared',{category:'item'});break;case 'skill':skill();break;case 'saveWarning':showSaveWarning();break;
...
$('#battle').addEventListener('pointerup',e=>{
  ...
  const ally=game.localAllies.find(a=>distance(a,pos)===0);if(ally){notify(`${allyName(ally)} · ${ally.status==='reforming'?'消散，重生倒數中':ally.status==='arriving'?'等候落點':ally.status==='destroyed'?'已毀':`HP ${ally.hp}/${ally.maxHp}${ally.kind==='drone'?(ally.payload?` · 裝填${GRENADES[ally.payload].name}`:` · 彈藥 ${ally.ammo}/${allyWeapon(ally).mag}`):''}`}。`);return;}
