...
- **英文命名決定（2026-09-16，使用者）**：叛軍的單位英文用 **Rogue X**（Rogue Rifleman／Rogue Breacher⋯），派系標籤仍是 **Rebel**。
  理由是設定：他們沒有離開設施，只是不再接受企業總部的指示，也沒有直接與企業為敵——英文的 *gone rogue* 正是這個意思；
  Rebel 太像「正在開戰」，Renegade 偏「倒戈」而且在 104px 的目標卡上會折成兩行（實測）。兩者並存也剛好表達立場差異：
  協定官方把他們分類成叛軍（Rebel），他們自己只是不再接電話（rogue）。忠誠者的單位直接用職務名（Rifleman、Assault、
  Breacher、Sniper、Warhound、Drone、Breaker）。目前遊戲沒有 i18n 層，這是給日後翻譯用的決定。
...
- **被感染槍兵 `rifleman_infected`**：步槍兵的變體。連發 5 發（步槍兵 3 發）、射擊命中 −35、染成綠色、只掉彈藥不掉武器。
- **被感染突擊兵 `raider_infected`**：突擊兵的變體。連發 6 發（突擊兵 4 發）、射擊命中 −35、染色、只掉手槍彈。
