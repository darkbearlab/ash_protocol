...
test('Recon starts with smoke and stun; public kill awards one smoke per level without another turn',()=>{
 ...
 p.xp=levelCost(g,1)-1;const turn=g.turn;g.rng=()=>0;/* 3.138.0: level costs come from levelCost */
...
test('multiple level gains spill every excess smoke, merge stacks, and survive save restore exactly once',()=>{
 const g=arena(),p=g.player;p.grenades=2;p.xp=levelCost(g,1)+levelCost(g,2)-1;const e=g.enemies[0];g.hurt(e,9999);
...
test('level 20 receives last smoke; capped XP rewards and other classes do not trigger it',()=>{
 const g=arena();g.player.level=19;g.player.xp=levelCost(g,19)-1;g.hurt(g.enemies[0],9999);assert.equal(g.player.level,20);assert.equal(g.player.smoke,3);
 ...
 const s=arena('soldier');s.player.xp=levelCost(s,1)-1;s.hurt(s.enemies[0],9999);assert.equal(s.player.level,2);assert.equal(s.player.smoke,0);
});
