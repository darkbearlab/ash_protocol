...
- **震撼彈（3.188.0，使用者 2026-09-26）**：擲彈兵兩種都會丟，準備投擲時決定：擲骰的最高三分之一是震撼彈（`AFFIX_TUNING.stunShare`）。
  - 預告與紀錄都寫明是震撼彈（「準備投震撼彈」「已投出震撼彈」；標記為黃色，預告寫「投擲震撼」、倒數寫「震撼 N」）；管制員的喊話照舊。
  - 範圍是落點周圍 3×3（手榴彈是十字 5 格），牆與關著的門照爆炸的規則擋住（`squareCells`）。
  - 不造成傷害；範圍內所有活物失能 4 回合（和玩家的震撼彈相同，照舊有 2 回合免疫），敵人、友軍也一樣。
  - 爆開的那一回合玩家若原地等待（防禦待機），失能減半為 2 回合；只在事後的紀錄寫出「你事先穩住，失能減半」，作戰指南「投擲物」一段補一句。失能抗性再減半。
  - 存檔：擲彈兵的準備與飛行中的標記多一個 `stun`（SAVE 77）；機器人遇到震撼標記時走出 3×3，走不出去就原地等待。
