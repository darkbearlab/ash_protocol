...
test('save44 maps old remaining duration by ceil/2 exactly once and preserves new half-decay progress; bad poison state rejects',()=>{
 assert.equal(SAVE_VERSION,74);const g=affixArena();for(let old=0;old<=6;old++){const raw=JSON.parse(g.serialize());raw.version=43;raw.data.player.poison=old;delete raw.data.player.poisonClock;const copy=Game.restore(JSON.stringify(raw));assert.ok(copy);assert.equal(copy.player.poison,Math.ceil(old/2));assert.equal(copy.player.poisonClock,undefined);assert.equal(Game.restore(copy.serialize()).player.poison,Math.ceil(old/2));}
