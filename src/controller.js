import {DRONE_REPAIR_COST,DRONE_REPAIR_FRACTION,droneRepairReason,ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon,PET_REGEN} from './allies.js';
...
$('#battle').addEventListener('pointerup',e=>{
  ...
  const ally=game.localAllies.find(a=>distance(a,pos)===0);if(ally){notify(`${allyName(ally)} · ${ally.status==='down'?'倒地，走到相鄰格以技能回收':ally.status==='destroyed'?'已毀，相鄰用僚機技能回收殘骸':`HP ${ally.hp}/${ally.maxHp}${ally.kind==='drone'?` · 彈藥 ${ally.ammo}/${allyWeapon(ally).mag}`:''}`}。`);return;}
