...
function showInventory() {
  ...
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><h2>帶走你需要的。</h2>
    ...
    <div class="pack-ammo">${AMMO_IDS.map(id=>`<span>${AMMUNITION[id].name}<b>${p[AMMUNITION[id].key]} / ${game.ammoCapacity(id)}</b></span>`).join('')}</div><p class="ammo-summary">備彈上限與彈匣分開計算。拾取只補到上限，多出的留在地上。<br>護甲板吸收直接傷害的至多一半，消耗後可拾取修復；不擋毒液或高熱。<br>換裝、拆解、改裝與補給各消耗 1 回合。查看背包不耗時。</p>
    <div class="pack-weapons">${p.owned.map(index=>{const w=WEAPONS[index],d=game.weaponDamage(index),active=index===p.weapon,level=p.upgrades[index],cost=25+level*15;return `<article class="pack-weapon ${active?'equipped':''}"><img src="./assets/${w.file}.svg" alt="${w.name}"><div><small>${active?'已裝備':'備用'} / ${w.code}</small><h3>${w.name}${level?` +${level}`:''}</h3><p>${w.desc}</p><span>傷害 ${d.min}–${d.max}${w.burst?' ×2':''} · 射程 ${w.range} · ${AMMUNITION[w.ammoType].name} · 彈匣 ${p.ammo[index]}/${w.mag}</span><div class="pack-actions"><button data-equip="${index}" ${active?'disabled':''}>裝備</button><button data-salvage="${index}" ${p.owned.length<=1?'disabled':''}>拆解 +${20+level*10}</button>${active?`<button data-bag-action="upgrade" ${p.scrap<cost||level>=3?'disabled':''}>${level>=3?'改裝已滿':`改裝 ${cost} 廢料`}</button>`:''}</div></div></article>`;}).join('')}</div>
