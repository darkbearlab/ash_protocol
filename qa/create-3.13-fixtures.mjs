...
function arena(name,portrait){const g=new Game(3130,[],0,'recon',portrait);g.runId=`qa-portrait-${name}`;g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));Object.assign(g.player,{x:10,y:10,hp:100,maxHp:100});g.enemies=[];g.items=[];g.props=[];g.hazards=[];g.marks=[];g.rooms=[];g.end={x:20,y:20};return g;}
