...
export function decodeBackup(raw,namespace){
  ...
  if(b.campaign!==null){
    ...
    requireValue(['hp','maxHp','meds','sprays','adrenaline','barricades','grenades','smoke','emp','stun','armor','bonus','blastBonus','healBonus','hazmat','scavenger','scrap','level','xp','kills','reserve','pistol','shell','energy','ordnance','poison'].every(k=>count(player[k]))&&Object.values(player.stats).every(count)&&count(game.pendingPerks),'任務角色數值無效。');
