...
test('exposed lateral movement matches stationary wall protection for every weapon affix, size and movement trait',()=>{
  const g=new Game(313);g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));g.props=[];g.player.x=10;g.player.y=10;g.player.traits=[];
...
test('player exposed sidestep is 55%, stacks with agility, ends with a paid action and adds no cover damage reduction',()=>{
  const g=new Game(313,[],0,'recon');g.grid=Array.from({length:SIZE},()=>Array(SIZE).fill(1));g.props=[];g.player.x=10;g.player.y=10;
