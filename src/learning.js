...
export function learningReason(g,id,dismantle=false){
 ...
 if(dismantle)return '';
 if(d.trait?hasTrait(p,d.trait):d.skills.every(s=>p.skills.includes(s)))return '已經學會，這份資料可拆解。';
...
export function useLearning(g,id,dismantle=false){
 ...
 else if(d.trait){if(!grantTrait(p,d.trait,`learned:${id}`))return false;}
...
export const learningInventory=g=>Object.entries(g.player.learningItems).map(([id,count])=>({id,count,...LEARNING_ITEMS[id],scrap:LEARNING_SCRAP,useReason:learningReason(g,id),dismantleReason:learningReason(g,id,true)}));
