...
import {read,write,loadGame,saveGame,storage,profile,recordResult,TEST_MODE,purchaseCarrying,exportBackup,previewBackup,restoreBackup} from './storage.js';
...
function showIntro(){modal(`<div class="eyebrow">ASH PROTOCOL / 灰燼協定</div><h2>深入寂靜。<br>活著回來。</h2><p>木星邊境的厄瑞玻斯設施停止回傳訊號。你是最後一名接近它的行動員。</p><p>深入六層設施，擊敗封鎖官與核心守衛，帶著紀錄撤離。只有你採取行動，敵人才會行動。</p><p>協定點數 ${profile().protocol.balance} · 探索資料、擊敗頭目與完成樓層可累積，死亡仍保留。現在可用於永久攜行升級。</p><div class="intro-rules"><span>↑ 四方向探索</span><span>⌖ 掩體與命中率</span><span>◷ 等待觀察</span><span>▣ 搜刮與換裝</span></div><p>${resumable||entered?`任務 ${game.seed} · 第 ${game.floor} 層 · ${game.turn} 回合`:'新任務已就緒。先尋找牆角，避免暴露在交叉火力中。'}</p><button class="modal-button" data-modal="enter">${resumable||entered?'繼續任務':'開始行動'} →</button><button class="modal-button secondary" data-modal="carrying">永久攜行升級</button><button class="modal-button secondary" data-modal="help">閱讀作戰指南</button>`);}
...
function showInventory() {
  ...
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><h2>帶走你需要的。</h2>
    <div class="pack-resources"><span>✚ 醫療 <b>${p.meds}</b></span><span>◉ 手榴彈 <b>${p.grenades}/${game.ammoCapacity('grenade')}</b></span><span>◇ 廢料 <b>${p.scrap}</b></span><span>▣ 護甲板 <b>${p.plates}/30</b></span></div>
...
function showCarrying(message=''){const p=profile(),level=p.upgrades.carrying,cost=CARRY_COSTS[level];modal(`<div class="eyebrow">PERMANENT EQUIPMENT</div><h2>攜行裝備 · ${level} / ${CARRY_COSTS.length}</h2>${message?`<p role="status">${escapeHTML(message)}</p>`:''}<p>協定點數 <strong>${p.protocol.balance}</strong>。升級永久保留，立即提高目前與未來任務的容量，不耗回合、不補充彈藥。</p><div class="pack-ammo">${Object.entries(AMMUNITION).map(([id,info])=>`<span>${info.name}<b>${capacity(id,level)}${cost!==undefined?` → ${capacity(id,level+1)}`:''}</b></span>`).join('')}</div><button class="modal-button" data-buy-carry="${level}" ${cost===undefined||p.protocol.balance<cost||!storage.available||storage.recoveryPending?'disabled':''}>${cost===undefined?'已達最高等級':`升級至 ${level+1} 階 · ${cost} 協定點數`}</button><p>彈匣容量不變。完整備份包含已購買的攜行等級與點數；單局存檔不會帶入永久升級。</p><button class="modal-button secondary" data-modal="close">返回</button>`);}
...
function newGame(seed){entered=true;game=new Game(seed,profile().unlocks.weapons,profile().upgrades.carrying);playback=null;renderer.game=game;renderer.camera={x:game.player.x,y:game.player.y};renderer.effects=[];cancelAim();lastStatus='playing';previousFloor=game.floor;$('#modal').close();update();floorToast();}
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.buyCarry!==undefined){try{purchaseCarrying(game,Number(b.dataset.buyCarry));update();showCarrying('攜行裝備已永久升級。');}catch(error){showCarrying(error.message);}return;}
