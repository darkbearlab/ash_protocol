...
- 單局 `ash-save`：save **v66**（`data.js` 的 `SAVE_VERSION`）。舊版 1～65 都能讀（`LEGACY_SAVE_VERSIONS` 自動推算），讀取前先存 `ash-save-v{N}-backup`。
