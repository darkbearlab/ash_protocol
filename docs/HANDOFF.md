...
| `src/comms.js` | 管制員通訊框（3.168.0）：`commsMarkup(line)`，左邊 SOUND ONLY 方框（之後換頭像）、右邊台詞，下方倒數白線（`armComms`，計時器關閉）；任務簡報（controller.js `showBriefing`）與戰場上的通訊層（`sayComms`）共用。紀錄鉤子 `COMMS_HOOKS`／`commsForLogs`（3.168.1）：新紀錄比對語言表句子，清單待填。見 STORY.md 第 8 節 |
