...
test('one bubble per unit: repeats count up, louder lines replace quieter ones, quieter ones wait',()=>{
 const b=new CalloutBoard(),first=b.add(seen('move'),0);
 assert.equal(b.add(seen('move'),100),first);assert.ok(bubbleText(first).endsWith('×2'));
...
test('the same cue from other units soon after merges, but danger lines keep their own bubbles',()=>{
 const b=new CalloutBoard(),a=b.add(seen('spotted','a1'),0);
 assert.equal(b.add(seen('spotted','a2'),500),a);assert.equal(a.count,2);
 assert.notEqual(b.add(seen('spotted','a3'),T.mergeMs+1),a,'after the merge window a new bubble appears');
 const g1=b.add(seen('grenade','a4'),0),g2=b.add(seen('grenade','a5'),10);assert.ok(g1&&g2&&g1!==g2);
 const h=new CalloutBoard(),west=h.add(heard('search','west'),0);assert.equal(h.add(heard('search','west'),10),west,'heard lines merge by direction');
});
