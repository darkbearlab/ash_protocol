...
| `src/comms.js` | 管制員通訊框（3.168.0）：`commsMarkup(line)`，左邊 SOUND ONLY 方框（之後換頭像）、右邊台詞，下方倒數白線（`armComms`，計時器關閉）；任務簡報（controller.js `showBriefing`）與戰場上的通訊層（`sayComms`）共用。紀錄鉤子 `COMMS_HOOKS`／`commsForLogs`（3.168.1）：新紀錄比對語言表句子，清單待填。訊息（3.168.2）`{speaker, expression, line, vars}` 或 `text`，`resolveComms` 解析；角色表 `COMMS_SPEAKERS`（名稱、各表情頭像，沒圖用 SOUND ONLY），不指定說話的人時用 `dutySpeaker`。見 STORY.md 第 8 節 |
