...
function terminalOfferGroups(){
  const p=game.player;
  return [
    {name:'醫療',rows:[{id:'heal',title:'醫療修復',detail:`回復 ${healingAmount(p,TERMINAL_TUNING.healAmount)} 生命並清除中毒。`},{id:'med',title:'醫療包 +1',detail:`持有 ${p.meds}，不設上限。`},{id:'spray',title:PREPARED_CATALOG.item.spray.name,detail:`持有 ${p.sprays}，不設上限。`}]},
    ...
    {name:'道具',rows:Object.entries(TERMINAL_ITEMS).filter(([id,offer])=>offer.sold!==false&&id!=='spray').map(([id,offer])=>{const entry=PREPARED_CATALOG.item[id],held=offer.wear?p.wearables.includes(offer.wear):p[offer.resource];return {id,title:entry.name,detail:offer.wear?(held?'已經有一件。':'佩戴型：戴上與脫下各 1 回合。'):`持有 ${held}，不設上限。`};})},
