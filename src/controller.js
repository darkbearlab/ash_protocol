...
const deployNotice=()=>runIsLive()?'<p>確認後會放棄目前任務，保留已賺點數與永久進度。</p>':'';
...
<fieldset class="character-choices mission-choices"><legend>任務 · 六層深入／三層往返</legend>${Object.entries(MISSIONS).map(([id,m])=>`<label class="character-choice"><input type="radio" name="mission" value="${id}" ${id===game.mission.id?'checked':''}><span><strong>${m.name}</strong><small>${m.text}</small></span></label>`).join('')}</fieldset>
<p>第 3 層仍需擊敗封鎖官。三層往返取件後回入口上樓，回程不補給；六層非核心任務可迴避核心守衛。指定殲滅目標以青色菱形標記。回收耗 1 回合，不占背包容量。</p>
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.modal){switch(b.dataset.modal){
    ...
    case 'deployOperator':{const value=$('#new-seed')?.value,seed=value!==undefined&&value!==''?Number(value):undefined;
      if(seed!==undefined&&(!Number.isInteger(seed)||seed<0||seed>999999999)){notify('種子需為 0–999999999 的整數。');return;}
