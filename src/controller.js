...
import {Game,FLOORS,FLOOR_INFO,PERKS,PACK_LIMIT,ENEMY_TYPES,LORE,enemyName,distance} from './engine.js';
...
function update(view=renderer.game) {
  ...
  $('#status-effects').textContent=[exposed?`暴露 ${exposed}`:threats.length?'掩護':view.cover.length?'牆 / 箱旁':'',p.moved?'移動':'',p.guard?'減傷 50%':'',p.plates?`護甲板 ${p.plates}`:'',p.focus?'瞄準 +15':'',p.evasive?'閃避 +15':'',p.poison?`中毒 ${p.poison}`:'',initiative(p)<0?'快速':initiative(p)>0?'緩速':''].filter(Boolean).join(' · ');
...
function showIntro(){modal(`<div class="eyebrow">ASH PROTOCOL / 灰燼協定</div><h2>深入寂靜。<br>活著回來。</h2><p>木星邊境的厄瑞玻斯設施停止回傳訊號。你是最後一名接近它的行動員。</p><p>深入六層設施，擊敗封鎖官與核心守衛，帶著紀錄撤離。只有你採取行動，敵人才會行動。</p><p>協定點數 ${profile().protocol.balance} · 探索資料、擊敗頭目與完成樓層可累積，死亡仍保留。現在可用於永久攜行升級。</p><div class="intro-rules"><span>↑ 四方向探索</span><span>⌖ 掩體與命中率</span><span>◷ 等待觀察</span><span>▣ 搜刮與換裝</span></div><p>${resumable||entered?`任務 ${game.seed} · 第 ${game.floor} 層 · ${game.turn} 回合`:'新任務已就緒。先尋找牆角，避免暴露在交叉火力中。'}</p><button class="modal-button" data-modal="${game.status==='playing'?'enter':'new'}">${game.status!=='playing'?'開始新任務':resumable||entered?'繼續任務':'開始行動'} →</button><button class="modal-button secondary" data-modal="carrying">永久攜行升級</button><button class="modal-button secondary" data-modal="help">閱讀作戰指南</button><button class="modal-button secondary" data-modal="settings">備份與設定</button>`);}
...
`;
  ...
  modal(`<div class="eyebrow">FIELD PACK / ${p.owned.length} OF ${PACK_LIMIT}</div><h2>作戰背包</h2>
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.modal){switch(b.dataset.modal){
    ...
    case 'restart':modal('<div class="eyebrow">REDEPLOY</div><h2>重新部署？</h2><p>目前任務將記為放棄，保留已賺點數與永久升級，再開始新任務。</p><label class="seed-field">地圖種子（留空隨機）<input id="new-seed" type="number" min="0" max="999999999" placeholder="例：2026" inputmode="numeric"></label><button class="modal-button" data-modal="new">確認，開始新任務</button><button class="modal-button secondary" data-modal="close">返回目前任務</button>');break;
