import {isNoncombatant,enemyDef} from './enemy-data.js';
...
const groups={danger:['scream','grenade','bombard','aim','attack'],affix:['affix_fast','affix_infrared','affix_night_vision','affix_suppressor','affix_grenadier'],tactical:['flee','move','cover','hold','reload','flank'],injury:['hit','wounded','critical','suppressed','pinned'],perception:['spotted','lost','search']};
...
export function receiveCallout(g,actor,kind,detail={}){
 ...
 if(kind==='state'&&cue!=='flee'){const memory=states.get(actor)||{};const category=CALLOUT_CUES[cue].category;if(memory[category]===cue)return null;memory[category]=cue;states.set(actor,memory);}
 if(Math.abs(actor.x-g.player.x)+Math.abs(actor.y-g.player.y)>CALLOUT_TUNING.hearingRadius)return null;
 const visible=g.teamVisible(actor),event={type:'callout',...(isNoncombatant(actor)?{voice:enemyDef(actor).voice}:{}),faction:enemyFaction(actor),cue,...CALLOUT_CUES[cue],visibility:visible?'visible':'heard',...(visible?{actorId:actor.id,enemyType:actor.type,name:enemyDisplayName(actor),position:{x:actor.x,y:actor.y}}:{direction:direction(g.player,actor)})};
