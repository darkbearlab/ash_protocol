...
test('Recon alone starts with learned, prepared, ready signal break and normal supplies',()=>{
  ...
  for(const id of ['soldier','bulwark']){const other=arena(id);assert.deepEqual(other.player.skills,[]);assert.equal(use(other),false);assert.equal(other.turn,1);}
});
...
test('v19 Recon migration adds a ready prepared skill without refilling or changing RNG',()=>{
  for(const character of ['soldier','recon','bulwark']){
    ...
    const expected=structuredClone(p);expected.skills=character==='recon'?['signal_break']:[];expected.prepared.skill=character==='recon'?'signal_break':null;
    expected.skillState=character==='recon'?{signal_break:{remaining:0,cooldown:0}}:{};assert.deepEqual(restored.player,expected);
  }
});
