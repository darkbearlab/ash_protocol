...
import {ALLY_SKILLS,allySkillState,canAllySkill,allyName,allyWeapon,droneCells,defaultDroneCell,deployLimit,lineLimit,TETHER,CARRY_DISTANCE,SUMMON_LIMIT,SUMMON_INTERVAL,SUMMON_TETHER,RALLY_TURNS,PET_TETHER,DRONE_HP,SENTRY_ARMOR} from './allies.js';
...
function showWorkshop(message=''){
  ...
  const row=i=>{const unit=p.productionLines[i];
    ...
    return `<div class="ground-loot"><strong>序列 ${i+1} · ${def.name}</strong><small>${def.text}部署時從你身上裝填手槍彈。</small><button data-workshop-deploy="${i}" ${reason?'disabled':''}>${reason||'部署 · 選位置 · 1 回合'}</button></div>`;};
...
function showBlueprints(){
  const p=game.player;
  modal(`<div class="eyebrow">WORKSHOP / BLUEPRINTS</div><h2>選擇藍圖</h2><p>廢料 ${p.scrap} · 序列 ${p.productionLines.length}/${lineLimit(p)}</p>${Object.entries(UNIT_BLUEPRINTS).map(([id,def])=>{const reason=buildReason(game,id);return `<button class="perk" data-workshop-blueprint="${id}" ${reason?'disabled':''}><strong>${def.name} · ${def.cost} 廢料 · 1 回合</strong><span>${def.text}<br>武器：內建輕機槍（武器掛載之後開放）<br>彈藥：手槍彈，部署時從你身上裝填${reason?`<br>${reason}`:''}</span></button>`;}).join('')}<button class="modal-button secondary" data-workshop>返回工坊</button>`,true);
}
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.workshopBlueprint){modalAction('buildUnit',{blueprint:b.dataset.workshopBlueprint});return;}
...
$('#battle').addEventListener('pointerup',e=>{
  ...
  const ally=game.localAllies.find(a=>distance(a,pos)===0);if(ally){notify(`${allyName(ally)} · ${ally.status==='reforming'?'消散，重生倒數中':ally.status==='arriving'?'等候落點':ally.status==='destroyed'?'已毀':`HP ${ally.hp}/${ally.maxHp}${ally.kind==='drone'?` · 彈藥 ${ally.ammo}/${allyWeapon(ally).mag}`:''}`}。`);return;}
