...
| `src/comms.js` | 管制員通訊框（3.168.0）：`commsMarkup(line)`，左邊 SOUND ONLY 方框（之後換頭像）、右邊台詞；任務簡報（controller.js `showBriefing`）先用，之後主選單等處共用。見 STORY.md 第 8 節 |
