...
import {SQUAD_TUNING,weaponAnswer,squadMembers,isSquadLeader,validSquad} from '../src/squad.js';
...
test('killing the leader ends the orders; changing weapon or moving away makes it start over',()=>{
  const {g,leader,squad}=scene({members:2});
  turns(g,2);
  ...
  Object.assign(g.player,{x:g.player.x+SQUAD_TUNING.regroup+1});g.action('wait');
  assert.ok(leader.squad.since>since,'moving out of the answer makes the squad redeploy');
...
test('squad orders survive a save, and a tampered order is refused',()=>{
  ...
  for(const mutate of [s=>s.state='charging',s=>s.weapon=5,s=>s.suppressTurn=-1,s=>s.at={x:99,y:0}]){
