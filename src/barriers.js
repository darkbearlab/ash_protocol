...
export const BARRIER_TYPES={
  door:{name:t('barrierTypes.door.name'),maxHp:60,openable:true,destructible:true,move:true,sight:true,shot:true,blast:true,cover:true},
  low_partition:{name:t('barrierTypes.low_partition.name'),maxHp:60,openable:false,destructible:true,vaultable:true,move:true,sight:false,shot:false,blast:false,cover:true},
  partition:{name:t('barrierTypes.partition.name'),maxHp:90,openable:false,destructible:true,move:true,sight:true,shot:true,blast:true,cover:true},
};
...
export const barrierName=b=>b.vault?t('barriers.vaultDoor'):b.vaultWall?t('barriers.vaultWall'):BARRIER_TYPES[b.type]?.name||t('barriers.obstacle');   // vault: 3.146.0; its walls: 3.177.2
...
export function validBarriers(edges,grid,ids=[]){
  ...
  for(const b of edges){
    ...
    // 3.146.0 (src/vault.js): only a vault door carries these, all three; a locked one is shut. 3.177.2: or a sealed
    // wall of its closet, which is never a door and carries exactly `indestructible` and `vaultWall`.
    if(b.vaultWall!==undefined){if(b.vaultWall!==true||b.indestructible!==true||b.type==='door'||b.vault!==undefined||b.locked!==undefined)return false;}
    else if((b.vault!==undefined||b.locked!==undefined||b.indestructible!==undefined)&&(b.type!=='door'||b.vault!==true||b.indestructible!==true||typeof b.locked!=='boolean'||b.locked&&b.open))return false;
