...
> **2026-09-15**：解鎖方向改由 [UNLOCKS.md](UNLOCKS.md) 取代。攜行升級將取消（點數全額退回）；本文「既有玩家不可倒退」一條，使用者已改成直接鎖回起始三職業。
...
3.90.0：攜行升級已取消並全額退款；「既有玩家不可倒退」的職業條款由使用者決定取代，舊檔鎖回 soldier/recon/engineer，進行中角色仍可打完。解鎖以 UNLOCKS 為準。
